		...
		public void CompleteOptLayerUpdate()
		{
			if (!_isUpdated || _layeredRequests.Count == 0)
			{
				...
				if(_isUpdatedPrev)
				{
					...
					//{
					//	Debug.Log("이전 프레임에서 업데이트 되었었다. > 마지막으로 Default 입력");
					//}
			...
			for (int iLayer = 0; iLayer < _layeredRequests.Count; iLayer++)
			{
					...
					|| _curLayeredRequest._blendMethod == apAnimPlayUnit.BLEND_METHOD.Interpolation)
				{
					switch (_valueType)
					{
						case TYPE.Int:
							...
							//Debug.Log("> Int Layer 0 : " + _curLayeredRequest.GetCalculated_Int() + " (" + _curLayeredRequest._totalWeight + ") >> " + _cal_Value_1D);
							break;
						...
						case TYPE.Float:
							...
							//Debug.LogWarning("> Float Layer 0 : " + _curLayeredRequest.GetCalculated_Float() + " (" + _curLayeredRequest._totalWeight + ") >> " + _cal_Value_1D);
							break;
				...
				//if(_valueType == TYPE.Vector2)
				//{
				//	Debug.Log(">> [" + iCalculatedLayer + "] > " + _cal_Value_2D);
				//}
				//else
				//{
				//	Debug.Log(">> [" + iCalculatedLayer + "] > " + _cal_Value_1D);
				//}
				...
				case TYPE.Int:
					{
						...
						//이전
						//_int_Cur = Mathf.RoundToInt(((float)_int_Cur * (1.0f - _totalWeight)) + ((float)_int_ClampedCalculatedLayer * _totalWeight));
						...
						//Debug.LogWarning(">> Result : " + _int_Cur);
					}
					break;
				...
				case TYPE.Float:
					{
						...
						//이전
						//_float_Cur = Mathf.Clamp((_float_Cur * (1.0f - _totalWeight)) + (_cal_Value_1D * _totalWeight), _float_Min, _float_Max);

						//변경 22.1.9 : Weight 0에 대한 값은 이전값이 아닌 기본값이다.
						_float_Cur = Mathf.Clamp((_float_Def * (1.0f - _totalWeight)) + (_cal_Value_1D * _totalWeight), _float_Min, _float_Max);
						//Debug.LogWarning(">> Result : " + _float_Cur);
					}
					break;
				...
				case TYPE.Vector2:
					{
						...
						//이전
						//_vec2_Cur.x = Mathf.Clamp((_vec2_Cur.x * (1.0f - _totalWeight)) + (_cal_Value_2D.x * _totalWeight), _vec2_Min.x, _vec2_Max.x);
						//_vec2_Cur.y = Mathf.Clamp((_vec2_Cur.y * (1.0f - _totalWeight)) + (_cal_Value_2D.y * _totalWeight), _vec2_Min.y, _vec2_Max.y);
						...
						//Debug.LogWarning(">> Result : " + _vec2_Cur);
					}
					break;
			}
		}
