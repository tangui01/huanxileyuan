...
namespace AnyPortrait
{
	...
	public class apUnityEvent
	{
		...
		public class TargetMethodSet
		{
			...
			public bool Validate(apUnityEvent.UNITY_EVENT_TYPE eventType)
			{
				_action_None = null;
				_action_Bool = null;
				_action_Int = null;
				...
				Type type_Target = _target.GetType();
				MethodInfo methodInfo = type_Target.GetMethod(_methodName, BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
				...
				//리턴 체크 (void 인지 체크한다.)
				if(!Type.Equals(methodInfo.ReturnType, typeof(void)))
				{
		...
		public static Type GetParamType(apAnimEvent.PARAM_TYPE paramType)
		{
			switch (paramType)
			{
