...
//#endif
		...
		public void UpdateBonesWorldMatrix(float tDelta, bool isPhysics)
		{
			if (_boneList_Root != null)
			{
				//1. World Matrix 처리
			...
			if (_childTransforms != null)
			{
				for (int i = 0; i < _childTransforms.Length; i++)
				{
					_childTransforms[i].UpdateBonesWorldMatrix(tDelta, isPhysics);
				}
			}
		}
		...
		public void UpdateBonesWorldMatrixAsSyncBones(float tDelta, bool isPhysics)
		{
			if (_boneList_Root != null)
			{
				//1. World Matrix 처리
				...
				{
					//_boneList_Root[i].MakeWorldMatrixForIK(true, false, isPhysics, tDelta);
					_boneList_Root[i].MakeWorldMatrixForIKAsSyncBones(true, false, isPhysics, tDelta);//Sync 용
				}
			...
			if (_childTransforms != null)
			{
				for (int i = 0; i < _childTransforms.Length; i++)
				{
					_childTransforms[i].UpdateBonesWorldMatrixAsSyncBones(tDelta, isPhysics);
				}
			}
		}
		...
		public void Initialize(bool isRecursive, bool isRoot)
		{
			...
			if(nBones > 0)
			{
				...
				{
					_boneList_All[iBone].Initialize();
				}
			}
			...
			if (isRecursive)
			{
				if (_childTransforms != null && _childTransforms.Length > 0)
				{
					for (int i = 0; i < _childTransforms.Length; i++)
					{
						_childTransforms[i].Initialize(true, false);
					}
				}
			}
		}
		...
		/// 대부분의 Link는 가능한 Bake에 시도하자.
		/// </summary>
		public IEnumerator InitializeAsync(bool isRecursive, bool isRoot, apAsyncTimer asyncTimer)
		{
			//ClearResultParams(false);
			...
			if (isRecursive)
			{
				if (_childTransforms != null && _childTransforms.Length > 0)
				{
					for (int i = 0; i < _childTransforms.Length; i++)
					{
						yield return _childTransforms[i].InitializeAsync(true, false, asyncTimer);
					}
				}
			}
		}
