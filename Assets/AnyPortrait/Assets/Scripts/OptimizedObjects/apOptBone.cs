...
namespace AnyPortrait
{
	...
	public class apOptBone : MonoBehaviour
	{
		...
		/// </summary>
		public void MakeWorldMatrixForIK(bool isRecursive, bool isCalculateMatrixForce, bool isPhysics, float tDelta)
		{
		...
		public void MakeWorldMatrixForIKAsSyncBones(bool isRecursive, bool isCalculateMatrixForce, bool isPhysics, float tDelta)
		{
			if (_syncBone == null)
			{
				if (_isIKCalculated_Controlled)
			...
			if(isRecursive)
			{
				if (_childBones != null)
				{
					for (int i = 0; i < _childBones.Length; i++)
					{
						_childBones[i].MakeWorldMatrixForIKAsSyncBones(true, isCalculateMatrixForce, isPhysics, tDelta);
					}
				}
			}
		}
