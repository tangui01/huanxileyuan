...
namespace AnyPortrait
{
	...
	public class apEditorController
	{
						...
						if (Editor.Select.Mesh != null)
						{
							...
							if (makeMeshMode != apEditor.MESH_EDIT_MODE_MAKEMESH_ADDTOOLS.Polygon)
							{
								...
								else
								{
									...
									apVertex clickedVert_Result = null;
									...
									for (int i = 0; i < nVertCount; i++)
									{
										...
										else if (IsVertexClickable_Wide(posGL, mousePos, ref curSqrDist))
										{
											//넓은 범위에서 클릭을 했다면 > 거리계산
									...
									{
										clickedVert_Result = clickedVert_Default;
									}
									else if (clickedVert_Wide != null)
									{
										//Debug.Log("완화된 클릭으로 버텍스 선택");
										clickedVert_Result = clickedVert_Wide;
									}

									//클릭한게 있다.
									if (clickedVert_Result != null)
									{
										if (prevSelectedVert == clickedVert_Result)
										{
											//같은걸 선택했다.
											//이동 가능
											_isMeshVertMovable = true;
										...
										Editor.VertController.SelectVertex(clickedVert_Result);
										isAnySelect = true;
							...
							if (_boneEdit_isMouseClickable && isMouseInGUI)
							{
								if (leftBtnStatus == apMouse.MouseBtnStatus.Down)
								{
									...
									if (Vector2.Distance(startPosW, endPosW) > 0.00001f)
									{
										...
										//Select 에 선택해주자
										Editor.Select.SetBone(newBone, apSelection.MULTI_SELECT.Main);
						...
						else
						{
							...
							if (_boneEdit_isMouseClickable && isMouseInGUI)
							{
								if (leftBtnStatus == apMouse.MouseBtnStatus.Down)
								{
									...
									{
										//처리가 끝났으면 Bone 교체
										Editor.Select.SetBone(targetBone, apSelection.MULTI_SELECT.Main);
									...
									string bakeDstFileRelativePath)
		{
			...
			Editor.Select.SetMeshGroup(rootMeshGroup);
		...
		public apAnimTimelineLayer AddAnimTimelineLayer(object targetObject, apAnimTimeline parentTimeline, bool isRecordAndRefresh = true)
		{
			...
			if (isRecordAndRefresh)
			{
				...
				Editor.Select.SetAnimTimelineLayer(newLayer, apSelection.MULTI_SELECT.Main, true);
		...
		public List<apAnimTimelineLayer> AddAnimTimelineLayersForMultipleSelection(apAnimTimeline parentTimeline, bool isRecordAndRefresh = true)
		{
			...
			if (isRecordAndRefresh)
			{
				...
				Editor.Select.SetAnimTimelineLayer(resultLayers[0], apSelection.MULTI_SELECT.Main, true);//생성된 첫번째 레이어를 메인으로 설정
				if(resultLayers.Count > 1)
				{
					for (int iLayers = 1; iLayers < resultLayers.Count; iLayers++)
					{
						//나머지 레이어들 추가
						Editor.Select.SetAnimTimelineLayer(resultLayers[iLayers], apSelection.MULTI_SELECT.AddOrSubtract, true);
					}
				}
				Editor.Select.AutoSelectAnimTimelineLayer(true, false);//<<타임라인 자동 스크롤 선택
		...
		public apAnimTimelineLayer AddAnimTimelineLayerForAllTransformObject(apMeshGroup parentMeshGroup, bool isTargetTransform, bool isAddChildTransformAddable, apAnimTimeline parentTimeline)
		{
			...
			Editor.Select.SetAnimTimelineLayer(firstLayer, apSelection.MULTI_SELECT.Main, true);
		...
		public void RemoveAnimTimelineLayer(apAnimTimelineLayer animTimelineLayer)
		{
			...
			{
				Editor.Select.SetAnimTimelineLayer(null, apSelection.MULTI_SELECT.Main, true);
			}
		...
		public apAnimKeyframe AddCopiedAnimKeyframe(int targetFrameIndex, apAnimTimelineLayer parentLayer, bool isMakeCurrentBlendData, apAnimKeyframe srcKeyframe, bool isRefresh, bool isRecord)
		{
			...
			if (srcKeyframe != null)
			{
				...
				if (newKeyframe._linkedModMesh_Editor != null && srcKeyframe._linkedModMesh_Editor != null)
				{
					...
					newKeyframe._linkedModMesh_Editor._transformMatrix.SetMatrix(srcTransformMatrix, true);
		...
		public void SelectVertexRigGrowOrShrink(bool isGrow)
		{
			...
			if (isGrow)
			{
				...
				Editor.Select.AddModVertRigs(addVertRigs);
			}
			else
			{
				//Shirink인 경우
		...
		public void SelectVerticesOfTheBone()
		{
			...
			if(!isCtrl)
			{
				...
				Editor.Select.SetModVertexOfModifier(null);//변경 20.6.28 : 변경된 MRV
			}
		...
		public apBakeResult Bake()
		{
			...
			//미리 안내를 하자
			if(!CheckIfAnyNoImageMesh(Editor._portrait))
			{
				//에러가 발생해서 Bake 취소
				return null;
			}
			...
			Transform curScaleCheckTransform = Editor._portrait.transform;
			while(true)
			{
			...
			Editor._portrait.LinkAndRefreshInEditor(false, apUtil.LinkRefresh.Set_AllObjects(null));
			...
			Editor._portrait.SetFirstInitializeAfterBake();//이게 호출되어야 Initialize가 제대로 동작한다.
			Editor._portrait.Initialize();

			//추가 20.8.10 [Flipped Scale 문제]
			//3.1 : 리깅 본 정보를 Initialize 직후에 Bake한다. (다만 옵션이 설정된 경우에 한해서)
			//Debug.LogError("Flipped Option : " + Editor._portrait._flippedMeshOption);
			if (Editor._portrait._flippedMeshOption == apPortrait.FLIPPED_MESH_CHECK.All)
			{
				for (int i = 0; i < optTransforms.Count; i++)
				{
					apOptTransform optTransform = optTransforms[i];
			...
			if(Editor._portrait._unityEventWrapper == null)
			{
				Editor._portrait._unityEventWrapper = new apUnityEventWrapper();
			}
			Editor._portrait._unityEventWrapper.Bake(Editor._portrait);
			...
			int nOptRootUnits = Editor._portrait._optRootUnitList != null ? Editor._portrait._optRootUnitList.Count : 0;
			if (nOptRootUnits > 1)
			{
				Editor._portrait.ShowRootUnitWhenBake(Editor._portrait._optRootUnitList[0]);
			}
		...
		public void GUI_Controller(int width, int height, int scrollY)
		{
			...
			if (nControlParam_Orders != nControlParam_RawData)
			{
				...
				for (int i = 0; i < cParams.Count; i++)
				{
					...
					{
						GUI_ControlParam(	iParamGUI, curParam, width,
		...
		private void GUI_ControlParam(	int index,
										apControlParam controlParam, int width,
										...
										int windowHeight, int windowScrollY)
		{
			...
			int rightBtnsWidth = isRecording ? 55 : 27;//50 + 5 / 25 + 2
				...
				))
			{
				...
				else
				{
					...
					//변경
					if (GUILayout.Button(Editor._guiContent_EC_MakeKey.Content, 
						apGUILOFactory.I.Width(recordBtnSize), apGUILOFactory.I.Height(recordBtnSize)))
					{
			...
			//Vector2 guiPos = new Vector2(lastRect.x, lastRect.y + 25);//기존 위치
				...
				case apControlParam.TYPE.Int:
					{
						...
						intNext = apControllerGL.DrawIntSlider(	guiPos + new Vector2(0, unitHeight + guiPosYOffset), width, 
																...
																controlParam, isRecording, recordKeyParamSet, recordedKey, animKeyframe);
				...
				case apControlParam.TYPE.Float:
					{
						...
						floatNext = apControllerGL.DrawFloatSlider(	guiPos + new Vector2(0, unitHeight + guiPosYOffset), width, 
																	width + 5, unitHeight + 10,
																	controlParam, 
																	isRecording, recordKeyParamSet, recordedKey, animKeyframe);
				...
				case apControlParam.TYPE.Vector2:
					{
						...
						//2차로 쉽게 제어
						vec2Next = apControllerGL.DrawVector2Slider(	guiPos + new Vector2(0, unitHeight + guiPosYOffset), width, unitHeight * 3, 
																		width + 5, (unitHeight * 3) + 20,
																		controlParam, isRecording, recordKeyParamSet,
																		recordedKey, animKeyframe);

						vec2Next.x = Mathf.Clamp(vec2Next.x, controlParam._vec2_Min.x, controlParam._vec2_Max.x);
						vec2Next.y = Mathf.Clamp(vec2Next.y, controlParam._vec2_Min.y, controlParam._vec2_Max.y);

						GUILayout.Space((unitHeight * 3) - 6);
						EditorGUILayout.BeginHorizontal(apGUILOFactory.I.Width(width));
			...
			//GUILayout.Space(20);//기존 여백
			//GUILayout.Space(10);//여백 줄임
