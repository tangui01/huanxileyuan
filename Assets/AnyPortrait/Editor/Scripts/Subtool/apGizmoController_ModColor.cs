		...
		public apGizmos.SelectResult Select__Modifier_ColorOnly(Vector2 mousePosGL, Vector2 mousePosW, int btnIndex, apGizmos.SELECT_TYPE selectType)
		{
			...
			if (!Editor.Select.IsSelectionLock)
			{
				...
				if (selectedMeshTransform != null)
				{
					...
					{
						Editor.Select.SetSubObjectInGroup(selectedMeshTransform, null, null, multiSelect, apSelection.TF_BONE_SELECT.Exclusive);//변경 20.5.27 : 다중 선택 옵션
					}
					else
					{
						...
						{
							Editor.Select.SetSubObjectInGroup(null, childMeshGroupTransform, null, multiSelect, apSelection.TF_BONE_SELECT.Exclusive);//변경 20.5.27 : 다중 선택
						}
						else
						{
							Editor.Select.SetSubObjectInGroup(selectedMeshTransform, null, null, multiSelect, apSelection.TF_BONE_SELECT.Exclusive);//변경 20.5.27 : 다중 선택
						}
					}
