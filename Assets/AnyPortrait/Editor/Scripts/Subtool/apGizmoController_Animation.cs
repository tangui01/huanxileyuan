	...
	public partial class apGizmoController
	{
		...
		public apGizmos.SelectResult FirstLink__Animation()
		{
			...
			if (isVertexSelectable)
			{
				...
				//변경 20.6.29
				if (Editor.Select.ModRenderVerts_All != null && workKeyframe != null && mainModMesh != null)
				{
			...
			if (isTransformSelectable)
			{
				...
				//{
				//	resultObj = Editor.Select.MeshTF_Main;
				//}
				//else if (Editor.Select.MeshGroupTF_Main != null)
				//{
				//	resultObj = Editor.Select.MeshGroupTF_Main;
				//}
				//else if (isBoneSelectable && mainModBone != null)
				//{
				//	resultObj = mainModBone._bone;
				//}
		...
		public apGizmos.SelectResult Select__Animation(Vector2 mousePosGL, Vector2 mousePosW, int btnIndex, apGizmos.SELECT_TYPE selectType)
		{
			...
			//apAnimTimelineLayer timelineLayer = Editor.Select.AnimTimelineLayer_Main;
			...
			//{
			//	prevSelectedObjs.Add(Editor.Select.MeshGroupTF_Main);
			//}
			//else if (Editor.Select.MeshTF_Main != null)
			//{
			//	prevSelectedObjs.Add(Editor.Select.MeshTF_Main);
			//}
			//else if (Editor.Select.Bone != null && isBoneTarget)
			//{
			//	prevSelectedObjs.Add(Editor.Select.Bone);
			//}
			...
			//apTransform_Mesh curSelectedMeshTransform = Editor.Select.MeshTF_Main;
			...
			//	Editor.Select.SetBoneForAnimClip(null, false, false, apSelection.MULTI_SELECT.Main);//자동 타임라인 레이어 선택은 생략
			//}
			////else
			////{
			////	//Vertex를 선택하지 않았다면
			...
			//			if (resultBone != null)
			//			{
			//				//Debug.LogError("TODO : 이거 Gizmo 확인해봐야함");

			//				//본이 선택되었다.
			//				Editor.Select.SetBoneForAnimClip(resultBone, true, true, multiSelect);

			//				//나머지는 Main일때만 null을 지정
			//				if (multiSelect == apSelection.MULTI_SELECT.Main)
			//				{
			//					Editor.Select.SetSubMeshTransformForAnimClipEdit(null, false, false, multiSelect);
			//					Editor.Select.SetSubMeshGroupTransformForAnimClipEdit(null, false, false, multiSelect);
			//				}

			//				//이전
			//				//selectObj = resultBone;

			//				//변경 20.5.27 : 다중 선택의 경우, 클릭한 본이 꼭 선택된 본은 아닐 수 있다.
			//				selectObj = Editor.Select.Bone;
			...
			//	}
			//	if (!isBoneSelected
			//		&& multiSelect == apSelection.MULTI_SELECT.Main//<<추가 20.5.27
			//		)
			//	{
			//		//본을 선택하지 않았으며 다중 선택이 아닐때
			//		//Editor.Select.SetBone(null);
			//		Editor.Select.SetBoneForAnimClip(null, true, false, apSelection.MULTI_SELECT.Main);
			//	}
			//}
			...
			//	if (isTransformTarget && !isSelectionCompleted)//같은 내용임
			//	{
			...
			//				Editor.Select.SetSubMeshTransformForAnimClipEdit(selectedMeshTransform, true, true, multiSelect);//<<수정 : 바뀐 여부 상관없이 그냥 선택한다.

			//				//다중 선택이 아닐 때에만 null을 지정한다.
			//				if (multiSelect == apSelection.MULTI_SELECT.Main)
			//				{
			//					Editor.Select.SetBoneForAnimClip(null, false, false, apSelection.MULTI_SELECT.Main);
			//					Editor.Select.SetSubMeshGroupTransformForAnimClipEdit(null, false, false, apSelection.MULTI_SELECT.Main);
			//				}
			...
			//		&& multiSelect == apSelection.MULTI_SELECT.Main//추가 20.5.27 : 추가 선택이 아닌 경우
			//		)
			//	{
			//		Editor.Select.SetSubMeshTransformForAnimClipEdit(null, true, false, apSelection.MULTI_SELECT.Main);
			//	}
			//}
			...
			////변경 20.5.27 : 선택된 것을 List로 정리해서 Multi로 넘기기
			...
			//	prevSelectedObjs.Add(Editor.Select.MeshTF_Main);
			//}
			//else if (Editor.Select.Bone != null && isBoneSelectable)
			//{
			//	prevSelectedObjs.Add(Editor.Select.Bone);
			//}
			...
			if (isVertexSelectable)
			{
					...
					&& nWorkKeyframes > 0)
				{
					...
					//1) 현재 선택한 Vertex가 클릭 가능한지 체크
					if (selectedMRV_Main != null)
					{
						if (selectedMRVs_All.Count == 1)
						{
							//1개인 경우
							if (Editor.Controller.IsVertexClickable(apGL.World2GL(selectedMRV_Main._renderVert._pos_World), mousePosGL))
							{
								if (selectType == apGizmos.SELECT_TYPE.Subtract)
								{
									//Editor.Select.RemoveModVertexOfAnim(modRenderVert_Main._modVert, modRenderVert_Main._renderVert);//이전
									Editor.Select.RemoveModVertexOfAnim(selectedMRV_Main);//변경 20.6.29
								}
						...
						else
						{
							...
							for (int iModRV = 0; iModRV < selectedMRVs_All.Count; iModRV++)
							{
								...
								if (Editor.Controller.IsVertexClickable(apGL.World2GL(curMRV._renderVert._pos_World), mousePosGL))
								{
									if (selectType == apGizmos.SELECT_TYPE.Subtract)
									{
										...
										//삭제인 경우
										//하나 지우자
										//Editor.Select.RemoveModVertexOfAnim(curMRV._modVert, curMRV._renderVert);
										Editor.Select.RemoveModVertexOfAnim(curMRV);//변경 20.6.29
									}
									else if (selectType == apGizmos.SELECT_TYPE.Add)
									{
										//Editor.Select.SetBoneForAnimClip(null);
									...
									else//if(selectType == apGizmos.SELECT_TYPE.New)
									{
										...
										//이전
										//apRenderVertex selectedRenderVert = curMRV._renderVert;
										//apModifiedVertex selectedModVert = curMRV._modVert;
										//Editor.Select.SetModVertexOfAnim(null, null);
										//Editor.Select.SetModVertexOfAnim(selectedModVert, selectedRenderVert);

										//변경
										Editor.Select.SetModVertexOfAnim(curMRV);
					...
					if (selectType == apGizmos.SELECT_TYPE.New)
					{
						//일단,  New 타입인데 클릭을 했으면 기존에 선택한건 날리자
						//Editor.Select.SetModVertexOfAnim(null, null);
						Editor.Select.SetModVertexOfAnim(null);//변경 20.6.29
					}
						...
						if(nMRVs > 0)
						{
							...
							for (int iMRV = 0; iMRV < nMRVs; iMRV++)
							{
								...
								if (isClick)
								{
									...
									{
										Editor.Select.AddModVertexOfAnim(curMRV);
									}
			...
			if (!isSelectionLock)
			{
				...
				//삭제 20.7.3 : 선택 해제는 아래에서 통합으로 처리한다.
				//if (!isBoneSelected
				//	&& multiSelect == apSelection.MULTI_SELECT.Main//<<추가 20.5.27
				//	&& nBones_Prev > 0//기존에 본을 선택했었다면
				//	)
				//{
				//	//본을 선택하지 않았으며 다중 선택이 아닐때
				//	//이전
				...
				//	Debug.LogError(">> Bone Deselected");
				//}
			}
			...
			if (!isSelectionLock)
			{
				...
				if (isTransformSelectable && !isSelectionCompleted)//같은 내용임
				{
						...
						selectType == apGizmos.SELECT_TYPE.Add)
					{
						...
						if (selectedMeshTransform != null)
						{
							...
							//변경 20.6.30 : 통합
							Editor.Select.SetSubObjectInGroup(selectedMeshTransform, null, null, multiSelect, apSelection.TF_BONE_SELECT.Exclusive);
				...
				//	&& isTransformSelectable
				//	&& multiSelect == apSelection.MULTI_SELECT.Main//추가 20.5.27 : 추가 선택이 아닌 경우
				//	&& nMeshTF_Prev > 0//추가. 이미 선택된 경우에
				//	)
				//{
				//	//이전
				//	//Editor.Select.SetSubMeshTransformForAnimClipEdit(null, true, false, apSelection.MULTI_SELECT.Main);
			...
			else
			{
				if (isVertexSelectable)
				{
					...
					else if (Editor.Select.ModRenderVerts_All.Count == 1)
					{
		...
		public void Unselect__Animation()
		{
			...
			else
			{
				...
				if ((int)(Editor.Select.AnimTimeline._linkedModifier.CalculatedValueType & apCalculatedResultParam.CALCULATED_VALUE_TYPE.VertexPos) != 0)
				{
					...
					Editor.Select.SetBoneForAnimClip(null, false, false, apSelection.MULTI_SELECT.Main);//이거 쓰자

					if (!Editor.Select.IsAnimSelectionLock)
					{
						Editor.Select.SetSubMeshTransformForAnimClipEdit(null, false, false, apSelection.MULTI_SELECT.Main);
						Editor.Select.SetSubMeshGroupTransformForAnimClipEdit(null, false, false, apSelection.MULTI_SELECT.Main);
					}
			...
			if (nModBones > 0)
			{
				...
				for (int iModBone = 0; iModBone < nModBones; iModBone++)
				{
						...
						|| (bone._parentBone._IKNextChainedBone != bone))
					{
						...
						//apComplexMatrix parentMatrix = null;//변경 20.8.12 : ComplexMatrix로 변경
						...
						//		//Render Unit의 World Matrix를 참조하여
						//		//로컬 값을 Default로 적용하자
						//		//parentMatrix = bone._renderUnit.WorldMatrixWrap;//기존

						//		//변경 20.8.13 : ComplexMatrix를 이용한다.
		...
		public void Move__Animation_Vertex(Vector2 curMouseGL, Vector2 curMousePosW, Vector2 deltaMoveW, int btnIndex, bool isFirstMove)
		{
			...
			//int nMRV = (Editor.Select.ModRenderVerts_All != null) ? Editor.Select.ModRenderVerts_All.Count : 0;
			////if (Editor.Select.ModRenderVerts_All == null)
			//if(nMRV == 0)
			//{
			...
			////if (Editor.Select.ModRenderVertListOfAnim.Count == 1 && !Editor.Gizmos.IsSoftSelectionMode)//이전
			//if (nMRV == 1 && Editor.Select.ModRenderVert_Main != null && !Editor.Gizmos.IsSoftSelectionMode)//변경 20.6.29
			//{
			//	targetVertex = Editor.Select.ModRenderVert_Main._renderVert;
			...
			if (Editor.Gizmos.IsSoftSelectionMode && Editor.Select.ModRenderVerts_Weighted.Count > 0)
			{
				...
				for (int i = 0; i < Editor.Select.ModRenderVerts_Weighted.Count; i++)
				{
					modRenderVert = Editor.Select.ModRenderVerts_Weighted[i];
					float weight = Mathf.Clamp01(modRenderVert._vertWeightByTool);
		...
		public void Scale__Animation_Vertex(Vector2 deltaScaleW, bool isFirstScale)
		{
			...
			if (Editor.Gizmos.IsSoftSelectionMode && Editor.Select.ModRenderVerts_Weighted.Count > 0)
			{
				for (int i = 0; i < Editor.Select.ModRenderVerts_Weighted.Count; i++)
				{
					...
					apRenderVertex renderVert = curMRV._renderVert;
		...
		public void TransformChanged_Position__Animation_Transform(Vector2 pos)
		{
			...
			////> 메인으로 부터 DeltaPos를 계산해서 작업해야하므로, 메인이 없으면 취소된다. (자동 키프레임 생성 이후에 처리할 것)
			//if(modMesh != null)
			//{
			//	targetObj = modMesh;
			//	deltaPos = pos - modMesh._transformMatrix._pos;
			//}
			//else if(modBone != null)
			//{
			//	targetObj = modBone;
			//	deltaPos = pos - modBone._transformMatrix._pos;
			//}
			//else
			//{
		...
		public void TransformChanged_Position__Animation_Vertex(Vector2 pos)
		{
			...
			//	for (int i = 0; i < Editor.Select.ModRenderVerts_All.Count; i++)
			//	{
			//		avgDeltaPosPrev += Editor.Select.ModRenderVerts_All[i]._modVert._deltaPos;
			//	}
			//	avgDeltaPosPrev /= Editor.Select.ModRenderVerts_All.Count;
		...
		private apHotKey.HotKeyResult OnHotKeyEvent__Animation_Vertex__Ctrl_A(object paramObject)
		{
			...
			if(selectableMRVs != null && selectableMRVs.Count > 0)
			{
				apSelection.ModRenderVert curMRV = null;
				for (int iMRV = 0; iMRV < selectableMRVs.Count; iMRV++)
				{
			...
			if(selectableMRVs != null && selectableMRVs.Count > 0)
			{
				...
				for (int iMRV = 0; iMRV < selectableMRVs.Count; iMRV++)
				{
					...
					if (isSelectable)
					{
						...
						{
							//제외한다.
							Editor.Select.RemoveModVertexOfAnim(curMRV);
						}
		...
		public bool SoftSelection_Animation_Vertex()
		{
			...
			//		if (Editor.Select.ModRenderVerts_All.Exists(delegate (apSelection.ModRenderVert a)
			//		 {
			...
			//		for (int iSelectedRV = 0; iSelectedRV < Editor.Select.ModRenderVerts_All.Count; iSelectedRV++)
			//		{
			//			apSelection.ModRenderVert selectedModRV = Editor.Select.ModRenderVerts_All[iSelectedRV];
			//			//현재 World위치로 선택해보자.
			...
			//				Editor.Select.ModRenderVerts_Weighted.Add(newModRenderVert);
			//			}
			//		}
			//	}
			//} 
		...
		public bool PressBlur_Animation_Vertex(Vector2 pos, float tDelta, bool isFirstBlur)
		{
			...
			if (Editor.Select.ModRenderVerts_All == null || Editor.Select.ModRenderVerts_All.Count <= 1 || linkedModifier == null)
			{
				return false;
			}
