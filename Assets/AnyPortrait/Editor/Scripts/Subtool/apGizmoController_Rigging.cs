		...
		public apGizmos.SelectResult Select__Modifier_Rigging(Vector2 mousePosGL, Vector2 mousePosW, int btnIndex, apGizmos.SELECT_TYPE selectType)
		{
				...
				)
			{
						...
						for (int iModRenderVert = 0; iModRenderVert < modRenderVerts.Count; iModRenderVert++)
						{
							...
							if (Editor.Controller.IsVertexClickable_Rigging(modRenderVert._renderVert, mousePosGL))
							{
								...
								else
								{
									...
									//변경 20.6.26
									Editor.Select.SetModVertexOfModifier(modRenderVert);
				...
				if (selectType == apGizmos.SELECT_TYPE.New)
				{
					...
					Editor.Select.SetModVertexOfModifier(null);//변경 20.6.26
				}

				#region [미사용 코드] 단일 선택, 구형 MRV
				//if (selectType != apGizmos.SELECT_TYPE.Subtract)
				//{
				//	if (
				...
				//		&& Editor.Select.ModMesh_Main._vertRigs != null
				//		)
				//	{
				//		//선택된 RenderUnit을 고르자
				...
				//				bool isClick = Editor.Controller.IsVertexClickable_Rigging(renderVert, mousePosGL);
				//				if (isClick)
				//				{
			...
			if (!isAnySelected && selectType == apGizmos.SELECT_TYPE.New)
			{
				...
				if (!isAnySelected)
				{
					...
					{
						Editor.Select.SetBone(prevBone, apSelection.MULTI_SELECT.Main);//복구
					}
				...
				else
				{
					...
					Editor.Select.AddModRenderVerts(prevSelectedVertices);
				}
			...
			if (!Editor.Select.IsSelectionLock)
			{
				if (!isAnySelected && selectType == apGizmos.SELECT_TYPE.New)
				{
					...
					if (selectedMeshTransform != null)
					{
						...
						{
							Editor.Select.SetSubMeshInGroup(selectedMeshTransform, apSelection.MULTI_SELECT.Main);
						}
						else
						{
							...
							{
								Editor.Select.SetSubMeshGroupInGroup(childMeshGroupTransform, apSelection.MULTI_SELECT.Main);
							}
							else
							{
								Editor.Select.SetSubMeshInGroup(selectedMeshTransform, apSelection.MULTI_SELECT.Main);
