...
public class UISprite : UIBasicSprite
{
	...
	[HideInInspector] [SerializeField] bool mIsGray = false;
	bool IsDownClick = false;
	public bool mIsDownClick {
		get { return mIsGray; }
		set {
			mIsGray = value;
		}
	...
	protected override void OnUpdate ()
	{
		...
		if(IsDownClick != mIsGray) {
			IsDownClick = mIsGray;
			string sn = IsDownClick ? (spriteName.Replace("_gray", "") + "_gray") : spriteName.Replace("_gray", "");
			spriteName = sn;
			this.GetComponent<BoxCollider>().enabled = !IsDownClick;
			UILabel[] labs = this.GetComponentsInChildren<UILabel>();
			foreach(UILabel lab in labs) {
				lab.color = IsDownClick ? Color.gray : this.color;
			}
		}
