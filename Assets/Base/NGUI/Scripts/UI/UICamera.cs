...
public class UICamera : MonoBehaviour
{
	...
	public void ProcessOthers ()
	{
		...
		if (GetAnyKeyDown != null ? GetAnyKeyDown() : Input.anyKeyDown || DealCommand.anyKeyDown)
		{
			for (int i = 0, imax = NGUITools.keys.Length; i < imax; ++i)
			{
				KeyCode key = NGUITools.keys[i];
				if (lastKey == key) continue;
				var mykey = DriverManager.GetRK3299Key(key);
				if (mykey != BgKeyCode.None)
				{
					if (!DealCommand.GetKeyDown(mykey)) continue;
				}
				else
				{
				...
				if(selectedObject) Notify(selectedObject, "OnKey", key); 
			}
