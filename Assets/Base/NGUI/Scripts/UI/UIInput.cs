...
public class UIInput : MonoBehaviour
{
	...
	protected virtual void Update ()
	{
		...
		if (mKeyboard != null)
		{
			string text = (mKeyboard.status==TouchScreenKeyboard.Status.Done || !mKeyboard.active) ? mCached : mKeyboard.text;
			...
			if (inputShouldBeHidden)
			{
				if (text != "|")
				{
					...
					else if (mKeyboard.status!=TouchScreenKeyboard.Status.Done && mKeyboard.active)
					{
						DoBackspace();
					}
					mKeyboard.text = "|";
			...
			{
				mCached = text;
				if (mKeyboard.status!=TouchScreenKeyboard.Status.Done && mKeyboard.active) value = text;
			}

			if (mKeyboard.status==TouchScreenKeyboard.Status.Done || !mKeyboard.active)
			{
				if (mKeyboard.status==TouchScreenKeyboard.Status.Canceled ) Submit();
				mKeyboard = null;
				isSelected = false;
				mCached = "";
			}
		...
		if (Input.GetKeyDown(KeyCode.Return) || Input.GetKeyDown(KeyCode.KeypadEnter) || Input.GetKeyDown((KeyCode)10))
		{
		...
		if (DealCommand.GetKeyDown(BgKeyCode.Left) )
		{
			if (validation == Validation.Unsigned)
			{
				int tmp;
				if (int.TryParse(mValue, out tmp))
				{
					if (!string.IsNullOrEmpty(mValue))
					{
		...
		//if (DealCommand.GetKeyDown(BgKeyCode.Down) )
		//{
		//	if (validation == Validation.Unsigned)
		//	{
		//		int tmp;
		...
		if (DealCommand.GetKeyDown(BgKeyCode.Right))
		{
			if (characterLimit == 0)
			{
				mValue += "0";
			...
			else if (mValue.Length < characterLimit)
			{
				mValue += "0";
				mSelectionStart = ++mSelectionEnd;
				UpdateLabel();
	...
	protected char Validate (string text, int pos, char ch)
	{
		...
		else if(validation == Validation.Unsigned) {
			// Unsigned number validation
			if(ch >= '0' && ch <= '9')
				return ch;
		}
	...
	public void SaveValue(string value) { SaveToPlayerPrefs(value); }
