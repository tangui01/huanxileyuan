...
public class UIScrollView : MonoBehaviour
{
	...
	public OnDragNotification onMove;

	public System.Action<Vector3> onRestrictStarted;
	...
	Vector3 mLocalScroll;
	Vector3 mStartPanelPosition;
	Vector3 mStartClipOffset;
	/// <summary>
	/// Move the scroll view by the specified local space amount.
	/// </summary>
	public Vector3 localScroll {
		get { return mLocalScroll; }
		set {
			mLocalScroll = value;
			mTrans.localPosition = mStartPanelPosition + mLocalScroll;
			mPanel.clipOffset = mStartClipOffset - mLocalScroll;
		}
	}
	...
	void Awake ()
	{
		...
		mStartPanelPosition = mTrans.localPosition;
		mStartClipOffset = mPanel.clipOffset;
	}
	...
	public bool RestrictWithinBounds (bool instant, bool horizontal, bool vertical)
	{
		...
		{
			onRestrictStarted?.Invoke(constraint);
	...
	public void ResetPosition(System.Collections.Generic.List<Transform> item, int index)
	{
		if(NGUITools.GetActive(this)) {
			// Invalidate the bounds
			mCalculatedBounds = false;
			...
			Bounds b = bounds;
			float heigh = b.size.y - mPanel.baseClipRegion.w + 10;
			float delta = (item[0].localPosition.y - item[index].localPosition.y) / heigh;

			// First move the position back to where it would be if the scroll bars got reset to zero
			SetDragAmount(0, delta, false);

			// Next move the clipping area back and update the scroll bars
			SetDragAmount(0, delta, true);

			mShouldMove = false;
		}
	}

	public float GetScrollPosition(System.Collections.Generic.List<Transform> item, int index)
	{
		mCalculatedBounds = false;
		Bounds b = bounds;
		float heigh = b.size.y - mPanel.baseClipRegion.w + 10;
		float delta = Mathf.Clamp((item[0].localPosition.y - item[index].localPosition.y - mPanel.baseClipRegion.w), 1, int.MaxValue) / heigh;
		return delta;
	}
	...
	public float GetScrollValue()
	{
		...
		contentPadding = contentMin + contentMax;
		return ((contentPadding > 0.001f) ? 1f - contentMin / contentPadding : 0f);
	}
