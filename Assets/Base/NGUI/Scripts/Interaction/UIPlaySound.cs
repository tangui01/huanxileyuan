...
public class UIPlaySound : MonoBehaviour
{
	...
	void Awake()
	{
		if(trigger == Trigger.OnPressRepeat) {
			UIButton btn = GetComponent<UIButton>();
			if(btn != null)
				btn.onClick.Add(new EventDelegate(() => { NGUITools.PlaySound(audioClip, volume, pitch); }));
			if(btn != null)
				btn.onPress.Add(new EventDelegate(() => { NGUITools.PlaySound(audioClip, volume, pitch); }));
		}
	}
}
