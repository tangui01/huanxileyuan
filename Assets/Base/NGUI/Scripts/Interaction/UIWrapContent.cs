...
public class UIWrapContent : MonoBehaviour
{
	...
	public void RebuildChildren()
	{
		...
		for(int i = 0; i < mTrans.childCount; ++i) {
			Transform t = mTrans.GetChild(i);
			if(hideInactive && !t.gameObject.activeInHierarchy)
				continue;
			mChildren.Add(t);
		}
	}
	...
	public Transform GetItem(GameObject current, Vector3 dir)
	{
		for(int i = 0; i < mChildren.Count; i++) {
			if(mChildren[i] == current.transform) {
				if(dir == Vector3.left || dir == Vector3.down) {
					return mChildren[(i + mChildren.Count + 1) % mChildren.Count];
				} else {
					return mChildren[(i + mChildren.Count - 1) % mChildren.Count];
				}
			}
