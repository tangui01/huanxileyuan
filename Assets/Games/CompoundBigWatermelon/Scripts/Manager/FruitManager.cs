...
{ 
	...
	public class FruitManager : IManager
	{
		...
		void UpdateFruitOperation()
		{
			...
			{
				UpdateCurFuritPos(false);
			}
			//J健
			if (DealCommand.GetKey(1, AppKeyCode.Flight))
			{
				UpdateCurFuritPos(true);
			}
			if (DealCommand.GetKeyUp(1, AppKeyCode.UpScore))
			{
				m_IsFalled = true;
				if (m_CurFruit != null)
				{
		...
		IEnumerator SpawnRandomFruit(Vector2 pos, Transform parent)
		{
			...
			if (CompoundBigWatermelonGameManager.Instance.GameOver == false)
			{
		...
		Fruit SpawnFruit(FruitSeriesType fruitSeriesType, Vector2 pos, Transform parent, bool isCompound)
		{
			...
			fruit.Init(fruitSeriesType, OnCompoundAction,m_AudioManager);
			if (isCompound == true)
			{
				fruit.EnableFruit();
			}
		...
		/// 实时更新对应的当前的水果位置
		/// </summary>
		private void UpdateCurFuritPos(bool isRight=false)
		{
			if (m_CurFruit != null && m_IsFalled == false)
			{
				Vector3 pos = isRight?m_CurFruit.transform.position+Vector3.right * (4 * Time.deltaTime):m_CurFruit.transform.position-Vector3.right * (4 * Time.deltaTime);
		...
		void CompoundHandle(Collision2D col, Fruit toColFruit)
		{
			...
			ScoreManager.Instance.AddScore(colFruitId * GameConfig.COMPOUND_FRUIT_BASE_SCORE);
		}
