...
{ 
	...
	public class LineManager : IManager
	{
        ...
        void UpdateWarninglineHandle() {
            m_Warningline.gameObject.SetActive(CompoundBigWatermelonGameManager.Instance.GameOverWarning);
        }
        ...
        void UpdateAimlineHandle() {
            ...
            else if (DealCommand.GetKey(1,AppKeyCode.UpScore))
            {
                UpdateAimlinePos();
            }
            else if (DealCommand.GetKeyUp(1,AppKeyCode.UpScore))
            {
                m_Aimline.gameObject.SetActive(false);
