/* Get inputs and assgin the delta distance to all ListBox1es.
 *
 * There are three controling modes:
 * 1. Free moving: Control the listBoxes with finger or mouse.
 *    You don't know where the ListBox1 would stop at.
...
public class ListPositionCtrl1 : MonoBehaviour
{
	...
	public static ListPositionCtrl1 Instance;
	/* Initial settings.
	...
	// Set the distance between each ListBox1. The larger, the closer.
	...
	/* Notice: ListBox1 will initialize its variables from here, so ListPositionCtrl1
	 * must be executed before ListBox1. You have to set the execution order in the inspector.
	 */
	void Start()
	{
		 ...
		 * ListPositionCtrl1 object is at the center).*/
		...
		// by scaling the gameObject ListPositionCtrl1 attached.
	...
	void storeMousePosition()
	{
		if (Input.GetMouseButtonDown( 0 )) {
			...
			foreach (ListBox1 listBox in listBoxes)
	...
	/* Move all ListBox1es for a distance which equals to the smallest distance
	 * between ListBox1 and the center.
	 * This method will be called from ListBox10 when needToAlignToCenter flag is set, and
