...
public class ListBox1 : MonoBehaviour
{
	...
	/* Notice: ListBox1 will initialize its variables from ListPositionCtrl1.
	 * Make sure that the execution order of script ListPositionCtrl1 is prior to
	 * ListBox1.
	 */
	void Start()
	{
		_canvasMaxPos = ListPositionCtrl1.Instance.canvasMaxPos_L;
		_unitPos = ListPositionCtrl1.Instance.unitPos_L;
		_lowerBoundPos = ListPositionCtrl1.Instance.lowerBoundPos_L;
		_upperBoundPos = ListPositionCtrl1.Instance.upperBoundPos_L;
		_rangeBoundPos = ListPositionCtrl1.Instance.rangeBoundPos_L;
		_shiftBoundPos = ListPositionCtrl1.Instance.shiftBoundPos_L;
	...
	/* Initialize the content of ListBox1.
	 */
	void initialContent()
	{
		if (listBoxID == ListPositionCtrl1.Instance.listBoxes.Length / 2)
			_contentID = 0;
		else if (listBoxID < ListPositionCtrl1.Instance.listBoxes.Length / 2)
			_contentID = ListBank.Instance.getListLength() - (ListPositionCtrl1.Instance.listBoxes.Length / 2 - listBoxID);
		else
			_contentID = listBoxID - ListPositionCtrl1.Instance.listBoxes.Length / 2;
...
//	}
	...
	public void setSlidingDistance( Vector3 distance, int slidingFrames )
	{
		...
		_slidingDistance = Vector3.Lerp( Vector3.zero, distance, ListPositionCtrl1.Instance.slidingFactor );
	}
	...
	public void unitMove( int unit, bool up_right )
	{
		...
		switch (ListPositionCtrl1.Instance.direction) {
		case ListPositionCtrl1.Direction.VERTICAL:
			setSlidingDistance( new Vector3( 0.0f, deltaPos.y, 0.0f ), ListPositionCtrl1.Instance.slidingFrames );
			break;
		case ListPositionCtrl1.Direction.HORIZONTAL:
			setSlidingDistance( new Vector3( deltaPos.x, 0.0f, 0.0f ), ListPositionCtrl1.Instance.slidingFrames );
			break;
		}
	}
	...
	void Update()
	{
		if (_keepSliding) {
			...
			if (_slidingFramesLeft == 0) {
				...
				if (ListPositionCtrl1.Instance.alignToCenter ||
					ListPositionCtrl1.Instance.controlByButton) {
				...
				if (listBoxID == 2 &&
					ListPositionCtrl1.Instance.needToAlignToCenter)
					ListPositionCtrl1.Instance.alignToCenterSlide();
				return;
			}

			updatePosition( _slidingDistance );
			_slidingDistanceLeft -= _slidingDistance;
			_slidingDistance = Vector3.Lerp( Vector3.zero, _slidingDistanceLeft, ListPositionCtrl1.Instance.slidingFactor );
		}
	}
	...
	void initialPosition( int listBoxID )
	{
		...
		case ListPositionCtrl1.Direction.VERTICAL:
			transform.localPosition = new Vector3( 0.0f,
				_unitPos.y * (float)( listBoxID * -1 + ListPositionCtrl1.Instance.listBoxes.Length / 2 ),
				0.0f );
			updateXPosition();
			break;
		case ListPositionCtrl1.Direction.HORIZONTAL:
			transform.localPosition = new Vector3( _unitPos.x* (float)( listBoxID - ListPositionCtrl1.Instance.listBoxes.Length / 2 ),
				0.0f, 0.0f );
			updateYPosition();
			break;
		}
	...
	/* Update the local position of ListBox1 accroding to the delta position at each frame.
	...
	void updateXPosition()
	{
		transform.localPosition = new Vector3(
			_canvasMaxPos.x * ListPositionCtrl1.Instance.angularity
	...
	void updateYPosition()
	{
		transform.localPosition = new Vector3(
			...
			_canvasMaxPos.y * ListPositionCtrl1.Instance.angularity
	...
	/* Check if the ListBox1 is beyond the upper or lower bound or not.
	 * If does, move the ListBox1 to the other side and update the content.
	 */
	void checkBoundaryY()
	{
	...
	void updateSize( float smallest_at, float target_value )
	{
		transform.localScale = _originalLocalScale *
			( 1.0f + ListPositionCtrl1.Instance.scaleFactor * Mathf.InverseLerp( smallest_at, 0.0f, Mathf.Abs( target_value )));
	}
	...
	/* Update to the last content of the next ListBox1
	 * when the ListBox1 appears at the top of camera.
	 */
	void updateToLastContent()
	{
		_contentID = nextListBox1.getCurrentContentID() - 1;
		_contentID = ( _contentID < 0 ) ? ListBank.Instance.getListLength() - 1 : _contentID;

	//	updateContent( ListBank.Instance.getListContent( _contentID ) );
	}

	/* Update to the next content of the last ListBox1
	 * when the ListBox1 appears at the bottom of camera.
	 */
	void updateToNextContent()
	{
		_contentID = lastListBox1.getCurrentContentID() + 1;
