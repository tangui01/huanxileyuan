/* Get inputs and assgin the delta distance to all ListBox2es.
 *
 * There are three controling modes:
 * 1. Free moving: Control the listBoxes with finger or mouse.
 *    You don't know where the ListBox2 would stop at.
...
public class ListPositionCtrl2 : MonoBehaviour
{
	...
	public static ListPositionCtrl2 Instance;
	/* Initial settings.
	...
	// Set the distance between each ListBox2. The larger, the closer.
	...
	/* Notice: ListBox2 will initialize its variables from here, so ListPositionCtrl2
	 * must be executed before ListBox2. You have to set the execution order in the inspector.
	 */
	void Start()
	{
		 ...
		 * ListPositionCtrl2 object is at the center).*/
		...
		// by scaling the gameObject ListPositionCtrl2 attached.
	...
	void storeMousePosition()
	{
		if (Input.GetMouseButtonDown( 0 )) {
			...
			foreach (ListBox2 listBox in listBoxes)
	...
	/* Move all ListBox2es for a distance which equals to the smallest distance
	 * between ListBox2 and the center.
	 * This method will be called from ListBox20 when needToAlignToCenter flag is set, and
