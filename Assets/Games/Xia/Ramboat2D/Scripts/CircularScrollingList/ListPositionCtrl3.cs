/* Get inputs and assgin the delta distance to all ListBox3es.
 *
 * There are three controling modes:
 * 1. Free moving: Control the listBoxes with finger or mouse.
 *    You don't know where the ListBox3 would stop at.
...
public class ListPositionCtrl3 : MonoBehaviour
{
	...
	public static ListPositionCtrl3 Instance;
	/* Initial settings.
	...
	// Set the distance between each ListBox3. The larger, the closer.
	...
	/* Notice: ListBox3 will initialize its variables from here, so ListPositionCtrl3
	 * must be executed before ListBox3. You have to set the execution order in the inspector.
	 */
	void Start()
	{
		 ...
		 * ListPositionCtrl3 object is at the center).*/
		...
		// by scaling the gameObject ListPositionCtrl3 attached.
	...
	void storeMousePosition()
	{
		if (Input.GetMouseButtonDown( 0 )) {
			...
			foreach (ListBox3 listBox in listBoxes)
	...
	/* Move all ListBox3es for a distance which equals to the smallest distance
	 * between ListBox3 and the center.
	 * This method will be called from ListBox30 when needToAlignToCenter flag is set, and
