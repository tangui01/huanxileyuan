...
public class SubmarineParkourPlayerManager : MonoBehaviour 
{
    ...
    static SubmarineParkourPlayerManager myInstance;
    ...
    public static SubmarineParkourPlayerManager Instance
    {
        get
        {
            if (myInstance == null)
                myInstance = FindObjectOfType(typeof(SubmarineParkourPlayerManager)) as SubmarineParkourPlayerManager;
    ...
    public void PlaySfxSound(int effectIndex, float volume = 1.0f)
    {
		AudioManager.Instance.SetEff(audioClips[effectIndex],volume,2);	    
    }
	...
	void OnTriggerEnter (Collider other)
	{
		...
		{	
			...
			if (other.transform.name == "Coin")
			{
				...
				AudioManager.Instance.SetEff(audioClips[2],1, 0);
                ...
                SubmarineParkourLevelManager.Instance.CoinGathered();
		...
		else if (other.tag == "PowerUps")
		{
			...
			UpdateMission(other.transform.name);
			AudioManager.Instance.SetEff(audioClips[3],1, 0);
	...
	void UpdateMission(string name)
	{
		...
		if (name == "Mine" || name == "MineChain" || name == "Chain")
			SubmarineParkourMissionManager.Instance.ObstacleEvent("Mine");
		else if (name == "Torpedo")
            SubmarineParkourMissionManager.Instance.ObstacleEvent("Torpedo");
		else if (name == "Laser" || name == "LaserBeam")
            SubmarineParkourMissionManager.Instance.ObstacleEvent("Laser");
		//Power up based update
		else if (name == "ExtraSpeed")
            SubmarineParkourMissionManager.Instance.PowerUpEvent("Extra Speed");
		else if (name == "Shield")
            SubmarineParkourMissionManager.Instance.PowerUpEvent("Shield");
		else if (name == "SonicWave")
            SubmarineParkourMissionManager.Instance.PowerUpEvent("Sonic Wave");
		else if (name == "Revive")
            SubmarineParkourMissionManager.Instance.PowerUpEvent("Revive");
	}
	//Play an explosion			
	void PlayExplosion(Transform expParent)
	{
		...
		if (expParent.name == "Torpedo")
		{
			...
			expParent.transform.parent.gameObject.GetComponent<SubmarineParkourTorpedo>().TargetHit(true);
		}
	...
	void ReviveCollected()
	{
		...
		hasRevive = true;
		SubmarineParkourGUIManager.Instance.RevivePicked();
        SubmarineParkourLevelGenerator.Instance.powerUpMain.DisableReviveGeneration();
	}
	...
	IEnumerator LaunchSonicWave()
	{
		...
		StartCoroutine(MoveToPosition(sonicWave.transform, new Vector3(65 + GameObject.Find("Move").transform.position.x, 0, -5), 1.25f, false));
		...
		sonicWave.GetComponent<SubmarineParkourSonicWave>().Disable();
	}
	...
	IEnumerator ExtraSpeedEffect(float time)
	{
        ...
        float newSpeed = SubmarineParkourLevelGenerator.Instance.scrollSpeed;
        SubmarineParkourLevelGenerator.Instance.scrollSpeed = 3;
        
        ParticleSystem VFX = transform.Find("Autumn aura").GetComponent<ParticleSystem>();
		//Wait for time
		double waited = 0;
		while (waited <= time)
		{
			...
			if (!paused)
				waited += Time.deltaTime;
			if (waited >= time - 0.35f)
			{
				ParticleSystem Yang = VFX.transform.Find("In Yang").GetComponent<ParticleSystem>();
				Yang.Stop();
			}
			//Wait for the end of the frame
			yield return 0;
        ...
        SubmarineParkourLevelGenerator.Instance.scrollSpeed = newSpeed;
		inExtraSpeed = false;
		canSink = true;
        ...
        SubmarineParkourLevelGenerator.Instance.ExtraSpeedOver();
	}
	//The sink effects
	IEnumerator SinkEffects()
	{
        ...
        SubmarineParkourLevelGenerator.Instance.StartCoroutine("StopScrolling", 2.5f);
	...
	IEnumerator MoveToPosition (Transform obj, Vector3 endPos, float time, bool enableSub) 
	{
		...
		if (enableSub)
		{
            ...
            for (int j = 0; j < bubbles.transform.childCount; j++)
            {
	            bubbles.transform.GetChild(j).GetComponent<ParticleSystem>().enableEmission = true;
            }
            bubbles.Play();
		}
	...
	public void ExtraSpeed(bool isCooldown = false)
	{
		...
		AudioManager.Instance.SetEff(audioClips[1],1, 1);
		ParticleSystem VFX = transform.Find("Autumn aura").GetComponent<ParticleSystem>();
		VFX.Play();
		var Root = GameObject.Find("Move");
		var Pos = Root.transform.position;
		Root.transform.DOMove(Pos+new Vector3(160,0,0),4f).SetEase(Ease.InOutQuad);
        ...
        SubmarineParkourLevelGenerator.Instance.ExtraSpeedEffect();
		StartCoroutine (ExtraSpeedEffect(4));
	}
	...
	public void MoveUp()
	{
		...
		{
			transform.Translate(new Vector3(0, 1.35f, 0));
		}
	}
	...
	{
		var viewPos = Camera.main.WorldToViewportPoint(thisTransform.transform.position);
		if (viewPos.x > 0.08f && subEnabled)
	...
	{
		if (hasRevive || (SubmarineParkourSaveManager.GetRevive() > 0 && !shopReviveUsed))
			return true;
		else
			return false;
	}
	...
	public IEnumerator Revive()
	{
		...
		if (!inRevive)
		{
			...
			if (hasRevive)
			{
				...
				hasRevive = false;
                SubmarineParkourGUIManager.Instance.DisableReviveGUI(0);
			}
			...
			else
			{
                ...
                SubmarineParkourGUIManager.Instance.DisableReviveGUI(1);
			}
			...
			StartCoroutine(MoveToPosition(this.transform, new Vector3(thisTransform.position.x-4f, -23, thisTransform.position.z), 1.0f, false));
			
			//Wait for 1.2 seconds, and restart level scrolling
			yield return new WaitForSeconds(1.2f);
            SubmarineParkourLevelGenerator.Instance.ContinueScrolling();
