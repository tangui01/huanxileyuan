...
public class SubmarineParkourLevelGenerator : MonoBehaviour 
{
	public SubmarineParkourTorpedoMain torpedoMain;								//A link to the torpedo manager
	public SubmarineParkourPowerUpMain powerUpMain;								//A link to the power up manager
    ...
    static SubmarineParkourLevelGenerator myInstance;
    ...
    public static SubmarineParkourLevelGenerator Instance
    {
        get
        {
            if (myInstance == null)
                myInstance = FindObjectOfType(typeof(SubmarineParkourLevelGenerator)) as SubmarineParkourLevelGenerator;
	...
	void Update()
	{
		...
		if (!paused)
		{
            ...
            SubmarineParkourMissionManager.Instance.DistanceEvent((int)distance);
		}
	}
	//Scroll the level
	void ScrollLevel()
	...
	IEnumerator StopScrolling (float time)
	{
        ...
        SubmarineParkourMissionManager.Instance.SinkEvent((int)distance);
		
		//Wait for a second
		yield return new WaitForSeconds(1);
		
		//If the player has a revive
        if (SubmarineParkourPlayerManager.Instance.HasRevive())
			//Show revive button
            StartCoroutine(SubmarineParkourGUIManager.Instance.ShowRevive());
		else
			//Else, show finish menu
            SubmarineParkourGUIManager.Instance.ShowEnd();
	}
	...
	{	
		...
		//Activate the object
		go.GetComponent<SubmarineParkourObstacles>().ActivateChild();
	}
