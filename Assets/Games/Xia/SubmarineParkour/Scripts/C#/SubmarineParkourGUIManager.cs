...
public class SubmarineParkourGUIManager : MonoBehaviour 
{
    public Renderer overlay;					//The overlay between the game and menus

    public Text bestDist; 					//Displays the best distance in the hangar
    ...
    public Text[] guiTexts;                 //The main GUI coin and distance indicator
    ...
    static SubmarineParkourGUIManager myInstance;
    ...
    public static SubmarineParkourGUIManager Instance
    {
        get
        {
            if (myInstance == null)
                myInstance = FindObjectOfType(typeof(SubmarineParkourGUIManager)) as SubmarineParkourGUIManager;
	...
	void Update()
	{
		...
		if (showMainGUI && !showPause)
		{
			...
			int currentScore = (int)(SubmarineParkourLevelGenerator.Instance.distance * 0.5f) + SubmarineParkourLevelManager.Instance.Coins() * 2;
			guiTexts[0].text = currentScore+"";
		}
	}
	...
	public void Pause()
	{
        ...
        if (SubmarineParkourPlayerManager.Instance.Crashed())
			return;
		
		//Set the game to pause
        SubmarineParkourPlayerManager.Instance.Pause();
        SubmarineParkourLevelManager.Instance.PauseGame();
		showPause = true;
	...
	{
		//Update coin display
		shopTexts[0].text = SubmarineParkourSaveManager.GetCoins().ToString();
		//Update extra speed display
		shopTexts[2].text = shopPrices[0].ToString();
		shopTexts[1].text = "x " + SubmarineParkourSaveManager.GetExtraSpeed();
		//Update shield display
		shopTexts[4].text = shopPrices[1].ToString();
		shopTexts[3].text = "x " + SubmarineParkourSaveManager.GetShield();
		//Update sonic wave display
		shopTexts[6].text = shopPrices[2].ToString();
		shopTexts[5].text = "x " + SubmarineParkourSaveManager.GetSonicWave();
		//Update revive display
		shopTexts[8].text = shopPrices[3].ToString();
		shopTexts[7].text = "x " + SubmarineParkourSaveManager.GetRevive();
	}
	//Buy a speed power up, if has the money
	void BuySpeed()
	{
		//If the playe can afford the power up
		if (SubmarineParkourSaveManager.GetCoins() >= shopPrices[0])
		{
			//Remove the coins from the account, and add the power up to the account
			SubmarineParkourSaveManager.SetCoins(SubmarineParkourSaveManager.GetCoins() - shopPrices[0]);
            SubmarineParkourSaveManager.ModifyExtraSpeedBy(1);
			//Update the shop, and notify the mission manager
			UpdateShop();
            SubmarineParkourMissionManager.Instance.ShopEvent("Extra Speed");
		}
	}
	...
	{
		//If the playe can afford the power up
		if (SubmarineParkourSaveManager.GetCoins() >= shopPrices[1])
		{
			//Remove the coins from the account, and add the power up to the account
			SubmarineParkourSaveManager.SetCoins(SubmarineParkourSaveManager.GetCoins() - shopPrices[1]);
            SubmarineParkourSaveManager.ModifyShieldBy(1);
			//Update the shop, and notify the mission manager
			UpdateShop();
            SubmarineParkourMissionManager.Instance.ShopEvent("Shield");
		}
	}
	...
	{
		//If the playe can afford the power up
		if (SubmarineParkourSaveManager.GetCoins() >= shopPrices[2])
		{
			//Remove the coins from the account, and add the power up to the account
			SubmarineParkourSaveManager.SetCoins(SubmarineParkourSaveManager.GetCoins() - shopPrices[2]);
            SubmarineParkourSaveManager.ModifySonicWaveBy(1);
			//Update the shop, and notify the mission manager
			UpdateShop();
            SubmarineParkourMissionManager.Instance.ShopEvent("Sonic Wave");
		}
	}	
	...
	{
		//If the playe can afford the power up
		if (SubmarineParkourSaveManager.GetCoins() >= shopPrices[3])
		{
			//Remove the coins from the account, and add the power up to the account
			SubmarineParkourSaveManager.SetCoins(SubmarineParkourSaveManager.GetCoins() - shopPrices[3]);
            SubmarineParkourSaveManager.ModifyReviveBy(1);
			//Update the shop, and notify the mission manager
			UpdateShop();
            SubmarineParkourMissionManager.Instance.ShopEvent("Revive");
		}
	}	
	...
	public void ActivateSpeed()
	{
		...
		startPowerAct = true;
        SubmarineParkourPlayerManager.Instance.ExtraSpeed();
        SubmarineParkourSaveManager.ModifyExtraSpeedBy(-1);
	}
	...
	public void ActivateSprint()
	{
		...
		//Set startPowerAct true, activate the power up, and remove it from the account
		SubmarineParkourPlayerManager.Instance.ExtraSpeed(true);
	}
	...
	void ActivateShield()
	{
		...
		//Set startPowerAct true, activate the power up, and remove it from the account
		startPowerAct = true;
        SubmarineParkourPlayerManager.Instance.RaiseShield();
        SubmarineParkourSaveManager.ModifyShieldBy(-1);
	}
	...
	void ActivateSonicWave()
	{
		...
		//Set startPowerAct true, activate the power up, and remove it from the account
		startPowerAct = true;
        SubmarineParkourLevelGenerator.Instance.powerUpMain.SetSonicBlastFirst();
        SubmarineParkourSaveManager.ModifySonicWaveBy(-1);
	}
	...
	void StartToPlay()
	{
        ...
        if (SubmarineParkourSaveManager.GetRevive() > 0)
		...
		//Start the level
        SubmarineParkourLevelManager.Instance.StartLevel();
	}
    ...
    IEnumerator Resume()
    {
        ...
        //Resume the game
        SubmarineParkourPlayerManager.Instance.Resume();
        SubmarineParkourLevelManager.Instance.ResumeGame();
    }
    //Hides the menu, and restarts the game
    IEnumerator Restart(float time = 0)
    {
        yield return new WaitForSeconds(time);
        GameObject.Find("Move").transform.position = new Vector3(0, 0, 0); 
        FindObjectOfType<SubmarineParkourPlayerManager>().PlayBgmSound();
        FindObjectOfType<SubmarineParkourManage>().UpdateCooldown();
	    //Fade the screen
        ...
        GameOverImage.GetComponent<Animator>().SetTrigger("Exit");
        ...
        SubmarineParkourLevelManager.Instance.Restart();
    }
    ...
    IEnumerator QuitToMain()
    {
        ...
        SubmarineParkourLevelManager.Instance.QuitToMain();
    }
	...
	public void SetLevelResolution()
	{
		...
		GameObject[] scale = {mainMenuElements[5], pauseElements[3], SubmarineParkourLevelGenerator.Instance.background.gameObject, SubmarineParkourLevelGenerator.Instance.sand.gameObject, overlay.gameObject};
		
		SubmarineParkourResolutionManager.Instance.SetResolutionSetting(scale, shopElements, left, right, SubmarineParkourLevelGenerator.Instance.hangar);
	}
	//Button Down event
	...
	public void ButtonUp(Transform button)
	{
		...
		switch (button.name)
		{
			...
			case "AudioEnabler":
				AuidoTriggerer(button.GetComponent<Renderer>().material);
				break;
	...
	public void ShowEnd()
	{
        ...
        SubmarineParkourMissionManager.Instance.Save();
        EnableDisable(finishMenu, true);
		
		//Get the current coin and distance data
        int currentDist = (int)SubmarineParkourLevelGenerator.Instance.distance;
        int currentCoins = SubmarineParkourLevelManager.Instance.Coins();
		...
		if (currentDist > SubmarineParkourSaveManager.GetBestDistance())
			//Set the current distance as the best distance
			SubmarineParkourSaveManager.SetBestDistance(currentDist);
		
		//Add the collected coins to the account
		SubmarineParkourSaveManager.SetCoins(SubmarineParkourSaveManager.GetCoins() + currentCoins);
		
		//Show the finish menu
		StartCoroutine(FadeScreen(0.4f, 0.7f));
		StartCoroutine(MoveMenu(finishMenu.transform, 0, 67, 0.55f, false));
		
		if (GameObject.Find("FinishScore"))
		{
			int currentScore = (int)(SubmarineParkourLevelGenerator.Instance.distance * 0.5f) + SubmarineParkourLevelManager.Instance.Coins() * 2;
			GameObject.Find("FinishScore").GetComponent<TextMesh>().text = "Score:"+currentScore;
		}
	...
	IEnumerator AllowRestart()
	{
		...
		GameOverImage.transform.GetChild(0).GetComponent<Text>().text = guiTexts[0].text;
		FindObjectOfType<SubmarineParkourPlayerManager>().PlaySfxSound(4);
	...
	public void ShowStartPowerUps()
	{
		...
		bool hasSpeed = SubmarineParkourSaveManager.GetExtraSpeed() > 0;
		bool hasShield = SubmarineParkourSaveManager.GetShield() > 0;
		bool hasSonicWave = SubmarineParkourSaveManager.GetSonicWave() > 0;
	...
	public void UpdateBestDistance()
	{
		int currentDist = (int)SubmarineParkourLevelGenerator.Instance.distance;
		int currentCoins = SubmarineParkourLevelManager.Instance.Coins();
		bestDist.text =(int)(currentDist*0.5f + currentCoins*2)+"";
	}
	...
	public IEnumerator ShowRevive()
	{
		...
		while (waited <= 20)
		{
			...
			if(reviveActivated)
			{
				...
				yield return new WaitForSeconds(0.5f);
                SubmarineParkourLevelManager.Instance.Revive();
	...
	public IEnumerator ShowMissionComplete(string text)
	{
		...
		StartCoroutine(MoveMenu(go.transform, -200, yPosTarget, 0.4f, false));
