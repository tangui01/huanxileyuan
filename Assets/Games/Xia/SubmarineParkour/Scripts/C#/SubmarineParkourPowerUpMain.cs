...
public class SubmarineParkourPowerUpMain : MonoBehaviour 
{
	...
	List<SubmarineParkourPowerUp> inactive 	 	= new List<SubmarineParkourPowerUp>();			//A list of the active power ups
	List<SubmarineParkourPowerUp> activated	 	= new List<SubmarineParkourPowerUp>();			//A list of the deactivated power ups
	...
	void Start()
	{
		...
		foreach (Transform child in transform)
		{
			...
			inactive.Add(child.GetComponent<SubmarineParkourPowerUp>());
		}
	}
	//Find and returns a compatible power up to spawn
	SubmarineParkourPowerUp FindCompatiblePowerUp()
	{
		int n = 0;
		...
		if (sonicBlastFirst)
		{
			...
			foreach (SubmarineParkourPowerUp item in inactive) 
			{
		...
		else
		{
			...
			if (!canGenerateRevive)
			{
				...
				SubmarineParkourPowerUp powerUp	= null;
	...
	public void GeneratePowerUp(float multiplyValue)
	{
		...
		SubmarineParkourPowerUp powerUp = FindCompatiblePowerUp();
	...
	public void ResetPowerUp(SubmarineParkourPowerUp sender)
	{
