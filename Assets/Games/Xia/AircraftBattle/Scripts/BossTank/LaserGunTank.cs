...
public class LaserGunTank : MonoBehaviour {
	...
	void Start () 
	{
		if(wholeParent.name.Contains("Small"))
			health = bossTank.laserGunHealth;
		else
			health = bossTank.laserGunHealth;
	...
	void TakeDamage(int damage)
	{
		...
		if(health<=0)
		{
			...
			if(leftWing)
			{
				if(bossTank.leftWingCount == 0 && wingNormal.gameObject.activeSelf)
				{
			...
			{
				if(bossTank.rightWingCount == 0 && wingNormal.gameObject.activeSelf)
				{
					//wingNormal.renderer.enabled = false;
					//wingDamaged.renderer.enabled = true;
					wingNormal.gameObject.SetActive(false);
