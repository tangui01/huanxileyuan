...
public class MainGun : MonoBehaviour {
	...
	void Start ()
	{
		...
		regularDamage = 50 + (LevelGenerator.currentStage-1)*10;
		//uniqueDamage = 150 + (LevelGenerator.currentStage-1)*10;
...
//	}
	...
	void FireAway()
	{
		...
		if(!fired)
		{
			fireEvent.dontFire = true;
			GetComponent<Animation>()["MainGunArrivingAnimation"].normalizedTime = 0;
			GetComponent<Animation>()["MainGunArrivingAnimation"].speed = 2;
			if(!GetComponent<Animation>().isPlaying)
				GetComponent<Animation>().Play();
//			//Invoke("CanFireAgainRegularAttack", 1.5f);
		}
