...
public class WingTurret : MonoBehaviour {
	...
	void Start () 
	{
		if(wholeParent.name.Contains("1"))
			health = bossPlane.wingTurretLargeHealth;
		else
			health = bossPlane.wingTurretSmallHealth;
		...
		damage = 30 + 10*(LevelGenerator.currentStage-1);
	}
	...
	void TakeDamage(int damage)
	{
		...
		if(health<=0)
		{
			...
			if(leftWing)
				bossPlane.leftWingCount--;
			else
				bossPlane.rightWingCount--;
			
			
			if(leftWing)
			{
				if(bossPlane.leftWingCount == 0 && normalWing.gameObject.activeSelf)
				{
					...
					{
						//EngineLaserGunLargeNormal.renderer.enabled = false;
						//EngineLaserGunLargeDamaged.renderer.enabled = true;
						EngineLaserGunLargeNormal.gameObject.SetActive(false);
						EngineLaserGunLargeDamaged.gameObject.SetActive(true);
						//EngineLaserGunLargeNormal.parent.Find("Elipse").gameObject.SetActive(false);
					}
					if(EngineLaserGunSmallNormal.gameObject.activeSelf)
					{
						//EngineLaserGunSmallNormal.renderer.enabled = false;
						//EngineLaserGunSmallDamaged.renderer.enabled = true;
						EngineLaserGunSmallNormal.gameObject.SetActive(false);
						EngineLaserGunSmallDamaged.gameObject.SetActive(true);
						//EngineLaserGunSmallNormal.parent.Find("Elipse").gameObject.SetActive(false);
					}
				}
			}
			else
			{
				if(bossPlane.rightWingCount == 0 && normalWing.gameObject.activeSelf)
				{
					//normalWing.renderer.enabled = false;
					//damagedWing.renderer.enabled = true;
					normalWing.gameObject.SetActive(false);
