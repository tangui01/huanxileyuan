...
public class WingLaserGun : MonoBehaviour {
	...
	void Start () 
	{
		if(wholeParent.name.Contains("Small"))
			health = bossPlane.wingLaserGunSmallHealth;
		else
			health = bossPlane.wingLaserGunLargeHealth;
	...
	void TakeDamage(int damage)
	{
		...
		if(health<=0)
		{
			...
			if(leftWing)
			{
				if(bossPlane.leftWingCount == 0 && wingNormal.gameObject.activeSelf)
				{
			...
			{
				if(bossPlane.rightWingCount == 0 && wingNormal.gameObject.activeSelf)
				{
					//wingNormal.renderer.enabled = false;
					//wingDamaged.renderer.enabled = true;
					wingNormal.gameObject.SetActive(false);
			...
			LaserGunEngineDamaged.parent.gameObject.SetActive(true);
