...
public class GiveDamageToTarget : MonoBehaviour
{
    ...
    protected IEnumerator OnTriggerStay2D(Collider2D other)
    {
        ...
        if (deadDamageToPlayer == DEALDAMAGETOPLAYER.FallTo && SuperCommandoGameManager.Instance.Player.velocity.y > 0)
            yield break;
        ...
        if (takeDamage != null)
        {
            ...
            {
                var facingDirectionX = Mathf.Sign(SuperCommandoGameManager.Instance.Player.transform.position.x - transform.position.x);
                var facingDirectionY = Mathf.Sign(SuperCommandoGameManager.Instance.Player.velocity.y);

                SuperCommandoGameManager.Instance.Player.SetForce(new Vector2(pushTargetForce.x * facingDirectionX,
