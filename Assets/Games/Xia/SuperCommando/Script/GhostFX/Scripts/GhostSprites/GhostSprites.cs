...
public class GhostSprites : MonoBehaviour
{
	...
	void Start ()
	{
        //yield return null; 
		allowGhost = false;
		spriteSorting = trailSize;
		// Reference the GameObject's SpriteRenderer for the trail list
		if(this.gameObject != null)
			character = spriteRenderer==null? gameObject.GetComponent<SpriteRenderer>(): spriteRenderer;
		...
		Vector3 position = spriteRenderer == null? gameObject.transform.position: spriteRenderer.gameObject.transform.position;
	...
	void Update ()
	{
		...
		for(int i = ghostList.Count - 1 ; i >= 0; i--){
			...
			ghostList[i].transform.position = new Vector3(ghostList[i].transform.position.x,
			                                              ghostList[i].transform.position.y, gameObject.transform.position.z +
			                                              i * 0.0001f);
    ...
    //    while (found)
    //    {
    //        GameObject ghostSpritesInThisGameObject = GameObject.Find(gameObject.name + " - GhostSprite");
    //        if (ghostSpritesInThisGameObject != null)
    //            Destroy(ghostSpritesInThisGameObject);
    //        else
    //            found = false;
...
//		scale.y *= (Mathf.Sign (transform.localScale.y)) * gameObject.transform.localScale.y;
		scale.y *= (Mathf.Sign (transform.localScale.y)) * Mathf.Abs(gameObject.transform.localScale.y);
		...
		g.GetComponent<SpriteRenderer> ().sortingLayerID = character.sortingLayerID;
		Debug.Log(character.sortingLayerID);
		g.GetComponent<SpriteRenderer> ().sortingOrder = GhostSpriteSortingOrder - spriteSorting;
		spriteSorting--;
		if (spriteSorting <= 0)
			spriteSorting = trailSize;
