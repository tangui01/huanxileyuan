...
public class InformationSign : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            case SetButton.Jump:
                SuperCommandoControllerInput.Instance.TurnJump(false);
                break;
            case SetButton.Weapon:
                SuperCommandoControllerInput.Instance.TurnRange(false);
                SuperCommandoControllerInput.Instance.TurnMelee(false);
                break;
            case SetButton.Dash:
                SuperCommandoControllerInput.Instance.TurnDash(false);
                break;
        }
    ...
    void CheckPlayerInRange()
    {
        ...
        if (detectZone == DetectZone.Circle)
            hit = Physics2D.CircleCast(transform.position, radius, Vector2.zero, 0, SuperCommandoGameManager.Instance.playerLayer);
        else
            hit = Physics2D.BoxCast(transform.position, new Vector2(1, detectHigh), 0, Vector2.zero, 0, SuperCommandoGameManager.Instance.playerLayer);
        if (hit)
        {
            if (!playerInArea)
            {
                ...
                {
                    case SetButton.Jump:
                        SuperCommandoControllerInput.Instance.TurnJump(true);
                        break;
                    case SetButton.Weapon:
                        SuperCommandoControllerInput.Instance.TurnRange(true);
                        SuperCommandoControllerInput.Instance.TurnMelee(true);
                        break;
                    case SetButton.Dash:
                        SuperCommandoControllerInput.Instance.TurnDash(true);
                        break;
                }
