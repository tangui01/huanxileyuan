...
public class SuperCommandoBlock : MonoBehaviour, ICanTakeDamage {
   ...
   public void BoxHit()
    ...
    IEnumerator BoxHitCo()
    {
        ...
        if (random != null)
        {
            ...
            SuperCommandoSoundManager.Instance.PlaySfx(soundSpawn, soundSpawnVolume);
        }
        ...
        if (blockTyle == BlockTyle.Destroyable)
        {
            if (random == null)     //only play destroy sound when there are no treasure to spawn
	            SuperCommandoSoundManager.Instance.PlaySfx(soundDestroy, soundDestroyVolume);
	...
	void CheckEnemiesOnTop(){
		...
		var hits = Physics2D.CircleCastAll (spawnPoint.position, sizeDetectEnemies, Vector2.zero, 0, 1 << 11);
		if (hits.Length > 0) {
            foreach (var hit in hits)
            {
                if (hit.collider.gameObject.GetComponent<SuperCommandoBlock>() == null)
                {
