	...
	void Update () {
		if (type == Type.Normal) {
			...
			if (hit) {
				...
				if (hit.collider.gameObject == SuperCommandoGameManager.Instance.Player.gameObject) {
					SuperCommandoGameManager.Instance.Player.TakeDamage (damage, Vector2.zero, gameObject, hit.point);
					SuperCommandoSoundManager.Instance.PlaySfx (soundKillPlayer);
				}
			} else
