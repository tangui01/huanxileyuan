...
public class BOSS_1 : BossManager, ICanTakeDamage, IListener
{
    ...
    void Update()
    {
        ...
        if (isDead || SuperCommandoGameManager.Instance.State != SuperCommandoGameManager.GameState.Playing || SuperCommandoGameManager.Instance.Player.isFinish || disapearing || isSuperAttacking || isFallingObjectAttack || isFlyingAttack || isTornadoAttacking || isBoomerangeAttacking)
        {
        ...
        if (moving)
        {
            ...
            if (hitTarget)
            {
                ...
                if (Mathf.Abs(hitTarget.transform.position.x - transform.position.x) < 0.3f && !SuperCommandoGameManager.Instance.Player.controller.collisions.below)
                {
                    allowChasing = false;
                }
        ...
        {
            if (Physics2D.CircleCast(MeleePoint.position, meleeDamageZone, Vector2.zero, 0, SuperCommandoGameManager.Instance.playerLayer))
            {
                MeleeAttack();
   ...
   [ReadOnly] public bool isMeleeAttacking = false;
    ...
    {
        var hit = Physics2D.CircleCast(MeleePoint.position, meleeDamageZone, Vector2.zero, 0, SuperCommandoGameManager.Instance.playerLayer);
        if (hit)
            SuperCommandoGameManager.Instance.Player.TakeDamage(givePlayerDamage, new Vector2(0, 3), gameObject, hit.point);
    ...
    IEnumerator DisappearShowCo()
    {
        //anim.SetBool("walk", false);
        disapearing = true;
         SuperCommandoSoundManager.Instance.PlaySfx(disappearSound);
        yield return new WaitForSeconds(1);
    ...
    {
         SuperCommandoSoundManager.Instance.PlaySfx(throwSound);
    ...
    {
         SuperCommandoSoundManager.Instance.PlaySfx(speedAttackSound);
        mulSpeed = Speed;
        isAttackSpeed = true;
    ...
    IEnumerator SuperAttackCo()
    {
        ...
        while (superAttackCurrentBulletNumber < attackTimes && SuperCommandoGameManager.Instance.State == SuperCommandoGameManager.GameState.Playing)
        {
            ...
            if (useWaveAnimation)
             ...
             SuperCommandoSoundManager.Instance.PlaySfx(superAttackSound);
    ...
    IEnumerator FallingObjectAttackCo()
    {
        ...
        RaycastHit2D checkAbovePlayer = Physics2D.Raycast(SuperCommandoGameManager.Instance.Player.transform.position + Vector3.up, /*(GameManager.Instance.Player.inverseGravity ? Vector2.down : */Vector2.up/*)*/, 100, layerGround);

        float spawnBulletFX = distancePlayerHeadFA;
        if (checkAbovePlayer)
        {
            if (Vector2.Distance(SuperCommandoGameManager.Instance.Player.transform.position, checkAbovePlayer.point) < distancePlayerHeadFA)
                spawnBulletFX = Vector2.Distance(SuperCommandoGameManager.Instance.Player.transform.position, checkAbovePlayer.point) - 0.6f;
        }

        //Debug.LogError(Vector3.up * spawnBulletFX);
        Instantiate(fallingAttackBullet, SuperCommandoGameManager.Instance.Player.transform.position + Vector3.up * spawnBulletFX /** (GameManager.Instance.Player.inverseGravity ? -1 : 1)*/, Quaternion.identity).Init(numberBulletFA, delayFA, speedFA, spaceFA);
        anim.SetTrigger("fallingAttack");
         SuperCommandoSoundManager.Instance.PlaySfx(soundBPrepareFA);
        //allowMoving = false;
        yield return new WaitForSeconds(delayFA - 0.1f);
        anim.SetTrigger("fallingAttackB");
         SuperCommandoSoundManager.Instance.PlaySfx(soundAttackFA);
        yield return new WaitForSeconds(1);
        isFallingObjectAttack = false;
    }
    #endregion
   ...
   [ReadOnly] public List<Vector3> flyPoints;
    ...
    IEnumerator FlyingAttackCo(bool throwStone = false, bool throwSpreadBullet = false)
    {
        ...
        else
        {
            isFlyingAttackPrepare = true;
             SuperCommandoSoundManager.Instance.PlaySfx(soundFlyingAttackPrepare, 0.7f);
            ...
            if (SuperCommandoGameManager.Instance.Player.isPlaying)
            {
                ...
                LookAtPlayer();
                var hit = Physics2D.Raycast(centerPoint.position, (SuperCommandoGameManager.Instance.Player.transform.position + Vector3.up - centerPoint.position), 100, SuperCommandoGameManager.Instance.groundLayer);
                if (hit)
                {
                    targetFlyingAttack = hit.point;
                }
                else
                    targetFlyingAttack = SuperCommandoGameManager.Instance.Player.transform.position + Vector3.up;

                 SuperCommandoSoundManager.Instance.PlaySfx(airAttackSound);
                ...
                Instantiate(FlyingAttackEndFX, transform.position, Quaternion.identity);
                 SuperCommandoSoundManager.Instance.PlaySfx(soundFlyingAttackExplosion);
                //check player in range and deal a damage
                var hitGround = Physics2D.CircleCast(centerPoint.position, 1, Vector2.zero, 0, SuperCommandoGameManager.Instance.playerLayer);
                if (hitGround)
                    SuperCommandoGameManager.Instance.Player.TakeDamage(flyingAttackDamage, new Vector2(0, 3), gameObject, hitGround.point);
            }
        }
    ...
    {
         SuperCommandoSoundManager.Instance.PlaySfx(spreadBulletAttackSound, 0.7f);
    ...
    public void AnimTornatoAttack()
    {
        ...
        if (TA_earthQuakeFX)
         ...
         SuperCommandoSoundManager.Instance.PlaySfx(TA_sound);
    }
    ...
    IEnumerator BoomerangAttackCo()
    {
        ...
        allowThrowObject = false;
         SuperCommandoSoundManager.Instance.PlaySfx(BA_soundBegin);
        ...
        anim.SetBool("boomerangAttack", false);
         SuperCommandoSoundManager.Instance.PlaySfx(BA_soundEnd);
        SetMoving(true);
    }
