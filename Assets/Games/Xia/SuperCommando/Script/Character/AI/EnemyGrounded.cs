...
public class EnemyGrounded : MonoBehaviour, ICanTakeDamage, IListener
{
    ...
    void Update()
    {
        ...
        if (!isPlaying || isSocking || !SuperCommandoGameManager.Instance.Player.isPlaying)
        {
            velocity.x = 0;
            return;
        }
    ...
    public virtual void LateUpdate()
    {
        ...
        {
            if (SuperCommandoGameManager.Instance.Player.gameObject.layer != LayerMask.NameToLayer("HidingZone"))
            {
                if (Mathf.Abs(Vector3.Distance(transform.position, SuperCommandoGameManager.Instance.Player.transform.position)) > finishDistance)
                {
                    transform.position = Vector3.MoveTowards(transform.position, SuperCommandoGameManager.Instance.Player.transform.position + new Vector3(0, offsetPlayerY, 0), chaseSpeed * Time.deltaTime);
                    _directionFace = transform.position.x > SuperCommandoGameManager.Instance.Player.transform.position.x ? -1 : 1;
            ...
            else
            {
                ...
                if (gravity == 0)
                {
                    ...
                    waitingAfterPlayerGoHidingZone = SuperCommandoGameManager.Instance.Player.transform.position + Vector3.up * 5;
                }
                else
    ...
    IEnumerator CheckMeleeAttack(float delay)
    {
        ...
        var hit = Physics2D.CircleCast(meleePoint.position, meleeAttackZone, Vector2.zero, 0, SuperCommandoGameManager.Instance.playerLayer);
    ...
    void AnimFire()
    {
        ...
        SuperCommandoSoundManager.Instance.PlaySfx(soundRangAttack);
    }
    ...
    void AnimSpawn()
    {
        ...
        SuperCommandoSoundManager.Instance.PlaySfx(soundSpawnttack);
    }
    ...
    public void TakeDamage(int damage, Vector2 force, GameObject instigator, Vector3 hitPoint)
    {
        ...
        if (instigator && instigator.GetComponent<SuperCommandoBlock>() != null)
            isDead = true;
    ...
    protected virtual void HitEvent()
    {
        ...
        SuperCommandoSoundManager.Instance.PlaySfx(hurtSound, hurtSoundVolume);
        //if (HurtEffect != null)
    ...
    protected virtual void Dead()
    {
        ...
        StopAllCoroutines();
        SuperCommandoSoundManager.Instance.PlaySfx(deadSound, deadSoundVolume);
    ...
    {
        isSocking = true;
        SetForce(SuperCommandoGameManager.Instance.Player.transform.localScale.x * pushForce.x, pushForce.y);
