...
public class BOSS_2 : MonoBehaviour, ICanTakeDamage
{
    ...
    private void Update()
    {
        if (isDead)
            return;
        transform.localScale = new Vector3(Mathf.Abs(transform.localScale.x) * (transform.position.x > SuperCommandoGameManager.Instance.Player.transform.position.x ? 1 : -1), transform.localScale.y, transform.localScale.z);
    }
    ...
    IEnumerator Attack(float delay)
    {
        ...
        while (SuperCommandoGameManager.Instance.State != SuperCommandoGameManager.GameState.Playing)
        {
            yield return null;
        }
        StartCoroutine(Attack(Random.Range(MinAttackTime, MaxAttackTime)));
    ...
    public void TakeDamage(int damage, Vector2 force, GameObject instigator, Vector3 hitPoint)
    {
        ...
        else
        {
            ...
            SuperCommandoSoundManager.Instance.PlaySfx(hurtSound);
        }
    }
