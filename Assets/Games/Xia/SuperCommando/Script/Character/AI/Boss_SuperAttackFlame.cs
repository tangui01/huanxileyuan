...
public class Boss_SuperAttackFlame : MonoBehaviour {
	...
	void Start () {
		...
		RaycastHit2D hit = Physics2D.Raycast (SuperCommandoGameManager.Instance.Player.transform.position + Vector3.up, /*GameManager.Instance.Player.inverseGravity ? Vector2.up : */Vector2.down, 10, layerGround);
		if (hit) {
...
//		Invoke ("TurnOn", timeOff);
	...
	void OnTriggerEnter2D(Collider2D other){
		...
		if (other.GetComponent<SuperCommandoPlayer> ()) {
			other.GetComponent<SuperCommandoPlayer> ().TakeDamage (damage, Vector2.zero, gameObject, Vector2.zero);
			hitPlayer = true;
			box2D.enabled = false;
		}
	}
