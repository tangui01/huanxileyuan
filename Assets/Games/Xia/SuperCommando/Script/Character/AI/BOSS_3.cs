...
public class BOSS_3 : MonoBehaviour,ICanTakeDamage
{
	...
	void Update () {	
		...
		if (attack) {
			...
			if (Vector2.Distance (transform.position, player.transform.position) < 0.1f || SuperCommandoGameManager.Instance.State == SuperCommandoGameManager.GameState.Dead) {
				BackUp ();
			}
		} else if (backup) {
    ...
    void OnTriggerStay2D(Collider2D other){
		var Player = other.GetComponent<SuperCommandoPlayer> ();
		if (Player == null)
			return;
