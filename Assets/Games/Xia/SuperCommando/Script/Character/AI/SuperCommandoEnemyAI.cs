...
public class SuperCommandoEnemyAI : MonoBehaviour, ICanTakeDamage {
	[Header("Behavior")]
	...
	public virtual void Update () {
		...
		if (isUseProjectile) {
			...
			if (hit) {
				if (hit.collider.gameObject.GetComponent<SuperCommandoPlayer> () != null)
					FireProjectile ();
			}
		}
	}
	...
	public virtual void LateUpdate(){
        ...
        }else if (moveFastWhenDetectPlayer)
        {
            ...
            if (Physics2D.Linecast(transform.position + Vector3.left * movingDetectPlayerDistance + Vector3.up * 0.5f, transform.position + Vector3.right * movingDetectPlayerDistance + Vector3.up * 0.5f, SuperCommandoGameManager.Instance.playerLayer))
            {
	...
	private void FireProjectile(){
        ...
        SuperCommandoSoundManager.Instance.PlaySfx(fireSound);
	}
	...
	public void TakeDamage (int damage, Vector2 force, GameObject instigator, Vector3 hitPoint)
	{
		...
		if (instigator.GetComponent<SuperCommandoBlock> () != null)
			isDead = true;
