...
public class SuperCommandoEnemy : MonoBehaviour,ICanTakeDamage, IListener, ICanFreeze, ICanBurn, ICanShock {
    [Header("Behavier")]
  ...
  //          gameObject.AddComponent<CanBeJumpOn>();
        ...
        switch (startBehavior)
        {
            case STARTBEHAVIOR.BURROWUP:
	            SuperCommandoSoundManager.Instance.PlaySfx(soundSpawn);
...
//			cooldownDetect = int.MaxValue;
	...
	public void SetEnemyState(ENEMYSTATE state){
		...
		if (enemyState == ENEMYSTATE.EATING && SuperCommandoGameManager.Instance.State != SuperCommandoGameManager.GameState.Playing)
			return;
	...
	public virtual void DetectPlayer(float delayChase = 0){
		...
		SuperCommandoSoundManager.Instance.PlaySfx(soundDetectPlayer);
	...
	protected IEnumerator DelayBeforeChasePlayer(float delay){
        ...
        if (detectPlayerAct == DETECTPLAYER.RushIntoPlayer)
        {
            ...
            rushIntoPlayerPoint = SuperCommandoGameManager.Instance.Player.transform.position + Vector3.up * 0.5f;
            rushIntoPlayerDirection = (rushIntoPlayerPoint - (Vector2)transform.position).normalized;
            SetEnemyState(ENEMYSTATE.RUSHINTOPLAYER);
            SuperCommandoSoundManager.Instance.PlaySfx(soundRushToPlayerAttack);
	...
	public virtual void Die(){
		...
		if (enemyEffect == ENEMYEFFECT.EXPLOSION) {
			...
			SuperCommandoSoundManager.Instance.PlaySfx (soundDieBlow);
		} else
			SuperCommandoSoundManager.Instance.PlaySfx (soundDie);
	}
