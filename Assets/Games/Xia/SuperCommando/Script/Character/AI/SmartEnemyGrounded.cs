...
public class SmartEnemyGrounded : SuperCommandoEnemy, ICanTakeDamage {
    ...
    public override void Update ()
	{
		...
		if (!isPlaying || isSocking || !SuperCommandoGameManager.Instance.Player.isPlaying || enemyEffect== ENEMYEFFECT.SHOKING) {
			velocity.x = 0;
			return;
		}
    ...
    public virtual void LateUpdate() {
        ...
        if (isPlayerDetected && (enemyType == ENEMYTYPE.INWATER || enemyType == ENEMYTYPE.FLY))
        {
            if (enemyState == ENEMYSTATE.WALK)
            {
                Vector2 targetPoint = (Vector2)SuperCommandoGameManager.Instance.Player.transform.position + Vector2.right * (Mathf.Abs(chasingOffset.x)) * (isFacingRight() ? -1f : 1f) + Vector2.up * chasingOffset.y;
