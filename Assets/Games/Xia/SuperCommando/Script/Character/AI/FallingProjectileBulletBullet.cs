...
public class FallingProjectileBulletBullet : MonoBehaviour, ICanTakeDamage {
	...
	void OnTriggerEnter2D(Collider2D other){
		...
		SuperCommandoSoundManager.Instance.PlaySfx (soundExplosion);

		var damage = (ICanTakeDamage)other.gameObject.GetComponent (typeof(ICanTakeDamage));
		if (damage == null || other.GetComponent<SuperCommandoPlayer>()) {

			if (damage != null && targetLayer == (targetLayer | (1 << other.gameObject.layer))) {
				damage.TakeDamage (damageToGive, Vector2.zero, SuperCommandoGameManager.Instance.Player.gameObject, Vector2.zero);
			}
