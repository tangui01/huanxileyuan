...
public class TruckleTrigger : MonoBehaviour {
    ...
    private void Update()
    {
        ...
        if (hit && (SuperCommandoGameManager.Instance.Player.isGrounded && (SuperCommandoGameManager.Instance.Player.transform.position.y > transform.position.y) || hit.collider.gameObject.GetComponent<SimpleGravityObject>()!=null))
        {
            if (!detected)
            {
