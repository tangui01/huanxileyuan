...
public class PlaySoundWhenObjMovingHelper : MonoBehaviour
{
    ...
    void CheckingMoving()
    {
        if (Vector3.Distance(lastPos, transform.position) != 0 && Vector2.Distance(transform.position, SuperCommandoGameManager.Instance.Player.transform.position) < playDistancePlayer)
        {
