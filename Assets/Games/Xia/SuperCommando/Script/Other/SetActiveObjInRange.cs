...
public class SetActiveObjInRange : MonoBehaviour, IListener{
   ...
   public List<Transform> listGameObjects;
    ...
    void CheckDistance()
    {
        ...
        {
            if (child != null)
                child.gameObject.SetActive(Vector2.Distance(child.transform.position, SuperCommandoGameManager.Instance.Player.transform.position) < distanceActiveContainer);
        }
