...
public class SimplePathedMoving : MonoBehaviour {
	...
	Vector3 CalculatePlatformMovement() {
		...
		if (percentBetweenWaypoints >= 1) {
			...
			fromWaypointIndex ++;
            if (Vector2.Distance(SuperCommandoGameManager.Instance.Player.transform.position, transform.position) < playSoundWhenPlayerInRange)
                SuperCommandoSoundManager.Instance.PlaySfx(soundWhenMoveNextTarget);
