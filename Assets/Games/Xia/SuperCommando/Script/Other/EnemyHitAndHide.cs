...
public class EnemyHitAndHide : MonoBehaviour, ICanTakeDamage
{
    ...
    public void AnimThrow()
    {
        if (isDead)
            return;
        SuperCommandoSoundManager.Instance.PlaySfx(soundThrow);
        ...
        Vector2 _dir = SuperCommandoGameManager.Instance.Player.transform.position + Vector3.up - throwPoint.position;
    ...
    public void TakeDamage(int damage, Vector2 force, GameObject instigator, Vector3 hitPoint)
    {
        ...
        if (isDead)
        {
            SuperCommandoSoundManager.Instance.PlaySfx(soundDead);
            StopAllCoroutines();
