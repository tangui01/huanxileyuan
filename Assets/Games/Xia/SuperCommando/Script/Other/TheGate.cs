...
public class TheGate : MonoBehaviour {
    ...
    IEnumerator CheckPlayerCo(){
        while (true)
        {
            ...
            var hit = Physics2D.CircleCast(transform.position, checkingRadius, Vector2.zero, 0, SuperCommandoGameManager.Instance.playerLayer);
            if (hit)
            {
                ...
                if (OpenGate())
                {
                    ...
                    SuperCommandoSoundManager.Instance.PlaySfx(soundOpen);
                    //open gate
                    while (true)
                    {
                ...
                {
                    hit = Physics2D.CircleCast(transform.position, checkingRadius, Vector2.zero, 0, SuperCommandoGameManager.Instance.playerLayer);
                    yield return new WaitForSeconds(0.1f);
                }
                buttonTrigger.SetActive(false);
            }
	...
	public bool OpenGate(){
        if (!SuperCommandoGameManager.Instance.isHasKey && isLocked)
        {
            SuperCommandoSoundManager.Instance.PlaySfx(soundLocked);
            FloatingTextManager.Instance.ShowText(lockMessage, transform.position);
            return false;
        }
        else if (SuperCommandoGameManager.Instance.isHasKey && isLocked)
        {
            
            SuperCommandoGameManager.Instance.isHasKey = false;
            isLocked = false;
            statusDoorSr.sprite = isLocked ? lockedSprite : openImage;
            SuperCommandoSoundManager.Instance.PlaySfx(soundUnlock);
