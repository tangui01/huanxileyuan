...
public class PressureSwitchEvent : MonoBehaviour, IStandOnEvent
{
    ...
    public void StandOnEvent(GameObject instigator)
    {
        ...
        StartCoroutine(StandOnCo(instigator.GetComponent<SuperCommandoPlayer>() != null));
    }
    ...
    IEnumerator StandOnCo(bool isPlayer)
    {
        ...
        SuperCommandoSoundManager.Instance.PlaySfx(soundOn);
        if (!isPlayer)
            yield break;

        while (SuperCommandoGameManager.Instance.Player.isGrounded) { yield return null; }
        state = false;
        anim.SetBool("state", state);
        if (targetObject)
            ...
            Debug.LogWarning("NEED SET TARGET TO THIS: " + gameObject.name);
        SuperCommandoSoundManager.Instance.PlaySfx(soundOff);
