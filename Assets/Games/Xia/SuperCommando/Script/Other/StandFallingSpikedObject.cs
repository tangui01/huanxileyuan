...
public class StandFallingSpikedObject : RaycastController,IListener
{
    ...
    Dictionary<Transform, SuperCommandoController2D> passengerDictionary = new Dictionary<Transform, SuperCommandoController2D>();
    bool isMoving = false;
    ...
    void Update()
    {
        ...
        if (!isWorked)
        {
            if (SuperCommandoGameManager.Instance.Player.controller.collisions.below && SuperCommandoGameManager.Instance.Player.controller.collisions.ClosestHit.collider.gameObject == gameObject)
                Work();
            if ((Mathf.Abs(transform.position.x - SuperCommandoGameManager.Instance.Player.transform.position.x) <= detectPlayerDistanceX) && (Mathf.Abs(transform.position.y - SuperCommandoGameManager.Instance.Player.transform.position.y) <= detectPlayerDistanceY) && (transform.position.y > SuperCommandoGameManager.Instance.Player.transform.position.y))
                Work();
        ...
        if (isMoving)
        {
            ...
            {
                hits = Physics2D.CircleCastAll(transform.position + Vector3.down * 1, 0.2f, Vector2.zero, 0, 1 << 9);
                if (hits.Length > 0)
                {
                    foreach (var hit in hits)
                    {
    ...
    IEnumerator WorkCo()
    {
        ...
        isMoving = true;
        SuperCommandoSoundManager.Instance.PlaySfx(soundFalling);
    }
    ...
    void MovePassengers(bool beforeMovePlatform)
    {
        foreach (PassengerMovement passenger in passengerMovement)
        {
            ...
            {
                passengerDictionary.Add(passenger.transform, passenger.transform.GetComponent<SuperCommandoController2D>());
            }
