...
public class AutoSetBoolAnim : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        while (true)
        {
            ...
            if(Vector2.Distance(SuperCommandoGameManager.Instance.Player.transform.position, transform.position ) < playSoundPlayerInRange)
            {
                SuperCommandoSoundManager.Instance.PlaySfx(state ? soundOpen: soundClose);
            }
        }
    }
}
