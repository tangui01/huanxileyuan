...
public class AutoSpawn : MonoBehaviour {
	...
	IEnumerator SpawnEnemy(float delay){
		...
		SuperCommandoSoundManager.Instance.PlaySfx (spawnSound, spawnSoundVolume);
		Instantiate (SpawnObjects [Random.Range (0, SpawnObjects.Length)], transform.position, Quaternion.identity);
		counter++;


		if (maxItemsSpawned > 0 && counter < maxItemsSpawned && SuperCommandoGameManager.Instance.State == SuperCommandoGameManager.GameState.Playing)
			StartCoroutine (SpawnEnemy (Random.Range (TimeMin, TimeMax)));
	}
}
