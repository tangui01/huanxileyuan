...
namespace SaucerFlying
{
    public class CharacterScroller : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            int currentCharacterIndex = SaucerFlyingCharacterManager.Instance.CurrentCharacterIndex;
            currentCharacterIndex = Mathf.Clamp(currentCharacterIndex, 0, SaucerFlyingCharacterManager.Instance.characters.Length - 1);
            ...
            characterAngleSpace = Mathf.PI * 2 / SaucerFlyingCharacterManager.Instance.characters.Length;
            currentAngle = currentCharacterIndex * characterAngleSpace;
            for (int i = 0; i < SaucerFlyingCharacterManager.Instance.characters.Length; i++)
            {
                int deltaIndex = i - currentCharacterIndex;

                GameObject character = (GameObject)Instantiate(SaucerFlyingCharacterManager.Instance.characters[i], centerPoint, Quaternion.Euler(originalRotation.x, originalRotation.y, originalRotation.z));
                SaucerFlyingCharacter charData = character.GetComponent<SaucerFlyingCharacter>();
        ...
        void Update()
        {
            ...
            // Update UI
            totalCoins.text = SaucerFlyingCoinManager.Instance.Coins.ToString();
            SaucerFlyingCharacter charData = currentCharacter.GetComponent<SaucerFlyingCharacter>();
            ...
            if (currentCharacter != lastCurrentCharacter)
            {
                ...
                else
                {
                    ...
                    if (SaucerFlyingCoinManager.Instance.Coins >= charData.price)
                    {
        ...
        IEnumerator SnapAndRotate(float snapDistance)
        {
            ...
            SaucerFlyingSoundManager.Instance.PlaySound(SaucerFlyingSoundManager.Instance.tick);
        ...
        {
            bool unlockSucceeded = currentCharacter.GetComponent<SaucerFlyingCharacter>().Unlock();
            if (unlockSucceeded)
            {
                ...
                SaucerFlyingSoundManager.Instance.PlaySound(SaucerFlyingSoundManager.Instance.unlock);
            }
        }

        public void SelectCharacter()
        {
            SaucerFlyingCharacterManager.Instance.CurrentCharacterIndex = currentCharacter.GetComponent<SaucerFlyingCharacter>().characterSequenceNumber;

            if (ChangeCharacter != null)
                ChangeCharacter(SaucerFlyingCharacterManager.Instance.CurrentCharacterIndex);
        }
    }
}