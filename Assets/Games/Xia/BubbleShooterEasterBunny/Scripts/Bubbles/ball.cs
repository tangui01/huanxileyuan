...
public class ball : MonoBehaviour
{
...
#else
        if (DealCommand.GetKey(1,(AppKeyCode)3))
#endif        
        {
            GameObject ball = gameObject;
            // Debug.Log(!ball.GetComponent<ball>().setTarget);
            if( !ClickOnGUI(NewBall.Instance.rayPosition) && !launched && !ball.GetComponent<ball>().setTarget && mainscript.Instance.newBall2 == null  && newBall && !Camera.main.GetComponent<mainscript>().gameOver && ( BubbleGamePlay.Instance.GameStatus == BubbleGameState.Playing || BubbleGamePlay.Instance.GameStatus == BubbleGameState.WaitForChicken ) )
            {
                Vector3 pos = Camera.main.ScreenToWorldPoint( NewBall.Instance.rayPosition);
                worldPos = pos;
    ...
    void FixedUpdate()
    {
        ...
        if(transform.position.y >= 5.25f && (transform.position.x <= -3.55f || transform.position.x >= 3.55f))
            Destroy(gameObject);
        if( stopedBall )
        {
    ...
    public void FindLight(GameObject activatedByBall)
    {
        ...
        foreach( Collider2D obj in fixedBalls )
        {
            ...
            if( i <= 10 )
            {
                if( ( obj.gameObject.tag == "light"  ) && BubbleGamePlay.Instance.GameStatus == BubbleGameState.Playing ) {
                    DestroySingle( obj.gameObject );
                    DestroySingle( activatedByBall );
                }
                else if( ( obj.gameObject.tag == "cloud"  ) && BubbleGamePlay.Instance.GameStatus == BubbleGameState.Playing ) 
                {
    ...
    void OnTriggerEnter2D( Collider2D other )
    {
        ...
        if( other.gameObject.name.Contains( "ball" ) && setTarget && name.IndexOf( "bug" ) < 0 )
        {
            ...
            {
                if( ( other.gameObject.tag == "black_hole" ) && BubbleGamePlay.Instance.GameStatus == BubbleGameState.Playing )
                {
                    AudioManager.Instance.playerEffect1(SoundBase.Instance.black_hole);
...
//             check if we collided with a right block and adjust our speed and rotation accordingly
    ...
    void DestroySingle( GameObject obj, float speed = 0.1f )
    {
        ...
        if( obj.tag == "light" )
        {
            AudioManager.Instance.playerEffect1(SoundBase.Instance.spark);
