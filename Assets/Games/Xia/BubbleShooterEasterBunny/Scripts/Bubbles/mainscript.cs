...
public class mainscript : MonoBehaviour {
    ...
    public int ComboCount
    {
        ...
        { 
            ...
            if( value > 0 )
            {
                AudioManager.Instance.playerEffect1(SoundBase.Instance.combo[Mathf.Clamp(value-1, 0 , 5)]);
                creatorBall.Instance.CreateBug( lastBall, value );
                if( value >= 6 )
                {
                    AudioManager.Instance.playerEffect1(SoundBase.Instance.combo[5]);
    ...
    void Awake(){
        ...
        currentLevel = NewBall.currTarget;
		stage = 1;
...
//		arcadeMode = InitScript.Arcade;
    ...
    void Start()
    {
        ...
        BubbleGamePlay.Instance.GameStatus = BubbleGameState.BlockedGame;
	...
	void Update () {
        ...
        if( BubbleGamePlay.Instance.GameStatus == BubbleGameState.Win )
        {
        ...
        if( checkBall != null &&( BubbleGamePlay.Instance.GameStatus == BubbleGameState.Playing || BubbleGamePlay.Instance.GameStatus == BubbleGameState.WaitForChicken ))
        {
...
//			}
	...
	//		createBall();
        ...
        if( arcadeMode && Time.time > ArcadedropDownTime && BubbleGamePlay.Instance.GameStatus == BubbleGameState.Playing )
        {
		            bounceCounter = 0;
        ...
        if( LevelData.mode == ModeGame.Vertical && TargetCounter >= 6 && BubbleGamePlay.Instance.GameStatus == BubbleGameState.Playing )
        {
            BubbleGamePlay.Instance.GameStatus = BubbleGameState.Win;
        }
        else if( LevelData.mode == ModeGame.Rounded && TargetCounter >= 1 && BubbleGamePlay.Instance.GameStatus == BubbleGameState.WaitForChicken )
            BubbleGamePlay.Instance.GameStatus = BubbleGameState.Win;
        else if( LevelData.mode == ModeGame.Animals && TargetCounter >= TotalTargets && BubbleGamePlay.Instance.GameStatus == BubbleGameState.Playing )
            BubbleGamePlay.Instance.GameStatus = BubbleGameState.Win;

        else if( LevelData.LimitAmount <= 0 && BubbleGamePlay.Instance.GameStatus == BubbleGameState.Playing && newBall == null )
        {
...
//        print( colorsDict.Count );
    ...
    IEnumerator CheckFreeChickenCor()
    {
        ...
        BubbleGamePlay.Instance.GameStatus = BubbleGameState.WaitForChicken;
...
//            print( "check free chicken " +  balls.transform.childCount);
        ...
        else if( finishGame )
        {
            ...
            if(chicken.GetComponent<SpriteRenderer>().sortingOrder<40)
				chicken.GetComponent<SpriteRenderer>().sortingOrder += 40;
