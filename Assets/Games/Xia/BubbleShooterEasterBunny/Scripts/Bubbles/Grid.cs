...
public class Grid : MonoBehaviour
{
                ...
                if( value.GetComponent<ball>() != null )
                {
                    if( !value.GetComponent<ball>().NotSorting  )
                    {
                     ...
                     //        value.GetComponent<SpriteRenderer>().sortingOrder = Mathf.FloorToInt( 1 / ( transform.position.y + 10 ) * 100 );
                        ...
                        if( value.tag == "chicken" ) value.GetComponent<SpriteRenderer>().sortingOrder = 2;
                    }
                }
    ...
    void Update()
    {
        if( busy == null )
        {
            ...
            {
                if( ( BubbleGamePlay.Instance.GameStatus == BubbleGameState.Playing || BubbleGamePlay.Instance.GameStatus == BubbleGameState.Win || BubbleGamePlay.Instance.GameStatus == BubbleGameState.WaitForChicken ) && LevelData.LimitAmount > 0 )
                {
