...
public class creatorBall : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        else
        {
            ...
            BubbleGamePlay.Instance.GameStatus = BubbleGameState.PreTutorial;
        }
        createMesh();
    ...
    {
        mainscript.Instance.currentLevel = NewBall.currTarget;// TargetHolder.level;
    ...
    IEnumerator MoveUpDownCor( bool inGameCheck = false )
    {
        ...
        if( BubbleGamePlay.Instance.GameStatus == BubbleGameState.BlockedGame )
            BubbleGamePlay.Instance.GameStatus = BubbleGameState.PreTutorial;
        else if( BubbleGamePlay.Instance.GameStatus != BubbleGameState.GameOver && BubbleGamePlay.Instance.GameStatus != BubbleGameState.Win )
    ...
    public GameObject createBall( Vector3 vec, BallColor color = BallColor.random, bool newball = false, int row = 1 )
    {
      ...
      //      rig.fixedAngle = true;
            ...
            if (BubbleGamePlay.Instance.GameStatus == BubbleGameState.Playing)
            {
                ...
                if ( range == 1)
                    b.GetComponent<ball>().SetBoost(BoostType.ColorBallBoost);
                if ( range == 2)
                    b.GetComponent<ball>().SetBoost(BoostType.FireBallBoost);
