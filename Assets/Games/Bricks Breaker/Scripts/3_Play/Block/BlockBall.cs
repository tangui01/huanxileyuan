...
public class BlockBall : BlockBase
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.CompareTag("Ball"))
        {
            ...
            BricksBreakerPoolManager.Despawn(BricksBreakerPoolManager.Spawn(CtrPool.instance.pFxBallGet, transform.position, Quaternion.identity),
                1f);
            fxLoop.gameObject.SetActive(false);
            BricksBreakerSoundManager.Instance.PlayEffect(SoundList.sound_play_sfx_addball);
            ...
            transform.DOMoveY(-6, value).SetEase(Ease.InCubic).OnComplete(() => { });
        }
    }
}
