...
public class BlockGroup : MonoBehaviour
{
    ...
    public void SetBlockGroup(int num = 0, int health = 0)
    {
        ...
        for (int i = 0; i < CtrBlock.instance.firstTransform.Count; i++)
        {
            ...
            if (bCount > i)
            {
                ...
                {
                    //Add Ball block
                    blockBase = BricksBreakerPoolManager.Spawn(CtrPool.instance.pBlockAddBall, Vector3.zero, Quaternion.identity)
                        .GetComponent<BlockBase>();
                }
                else
                {
                    //Basic block
                    blockBase = BricksBreakerPoolManager.Spawn(CtrPool.instance.pBlockDefault, Vector3.zero, Quaternion.identity)
                        .GetComponent<BlockBase>();
                }
    ...
    {
        id += 1;
        transform.DOMove(transform.position + Vector3.down, time).SetEase(Ease.InOutSine);

        for (int i = 0; i < blockBases.Count; i++)
        {
            if (blockBases[i].transform.position.y<=CtrBlock.instance.transfomY[CtrBlock.instance.transfomY.Length-1].position.y)
            {
                transform.DOKill();
                Destory();
