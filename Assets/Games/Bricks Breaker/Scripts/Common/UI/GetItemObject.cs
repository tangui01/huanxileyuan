...
public class GetItemObject : MonoBehaviour
{
    ...
    private void Start()
    { 
        BricksBreakerPoolManager.CreatePool(pItem, 20, false, 0);
    }
    ...
    public void GetItem(int value, Vector2 f)
    {
        ...
        switch (itemGetType)
        {
            case ItemGetType.Coin:
                ...
                BricksBreakerSoundManager.Instance.PlayEffect(SoundList.sound_common_sfx_coin);
                break;
            case ItemGetType.Gem:
                last = GetComponent<GetItem>().posGem.position;
                BricksBreakerSoundManager.Instance.PlayEffect(SoundList.sound_common_sfx_gem);
                break;
        }
    ...
    IEnumerator SpwanCoinCo()
    {
        ...
        GameObject item = BricksBreakerPoolManager.Spawn(pItem, posf, Quaternion.identity);
        ...
        BricksBreakerPoolManager.Despawn(item, 0.5f);
