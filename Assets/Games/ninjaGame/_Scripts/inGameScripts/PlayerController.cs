...
//		public float JumpSpeed = 0.5f;
		...
		void Start()
		{
			...
			playerMaterial.mainTexture=playertexture[UnityEngine.Random.Range(0,playertexture.Length)];
		}
		...
		void FixedUpdate()
		{
			...
			switch (presentPlayerState)
			{
				...
				case playerStates.dead:
					...
					if (!oneTimeReset)
					{
						CameraController.instance.CameraShakeByDead();
						oneTimeReset = true;
		...
		IEnumerator PalyerAttackEf()
		{
			AttackEf.gameObject.SetActive(true);
			swordCollider.enabled = true;
			yield return	new WaitForSeconds(0.2f);
			AttackEf.gameObject.SetActive(false);
		...
		void OnTriggerEnter(Collider hit)
		{
			...
			{
				GameController.instance.CreateNewWorld();
		...
		void OnControllerColliderHit(ControllerColliderHit incoming)
		{
			...
			if (incoming.collider.CompareTag("Monster") && !ShieldIcon)
			{
				...
				GameController.instance.isplayerDaed = true;
				...
				GameObject obj =
					Instantiate(PlayerDeadEffect, thisTrans.position,
						Quaternion.identity) as GameObject; //to player dead effect
			...
			else if (incoming.collider.tag.Contains("Obstacles") && !ShieldIcon)
			{
				...
				sword.SetActive(false); //to sword deactive when player is dead
				GameObject.Find("Main Camera").GetComponent<CameraController>().enabled = true; //for camera shaking
			}
			//for player hit Rock when shield is off
			else if (incoming.collider.CompareTag("Rock") && !ShieldIcon)
			{
			...
			if (incoming.collider.CompareTag("Monster") && (ShieldIcon || !isPlayerOnTheGround))
			{

				MonsterController monster = incoming.collider.GetComponent<MonsterController>();
				monster.DeathState();
			...
			else if (incoming.collider.name.Contains("broken_pot") ||
			         incoming.collider.name.Contains("broken_pot") && ShieldIcon)
			{
