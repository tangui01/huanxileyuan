	...
	public class ObsticlesController : MonoBehaviour
	{
		...
		void OnTriggerEnter(Collider Hit)
		{
			...
			if (Hit.GetComponent<Collider>().name.Contains("sword"))
			{
				...
				GameObject obj =
					Instantiate(BrokenObj, gameObject.transform.position,
						Quaternion.identity) as GameObject; //for broken pot
				Destroy(obj, 0.5f); //to destroy the instantiate obj
				PlayerController.thisPosition = new Vector3(PlayerController.thisPosition.x,
					PlayerController.thisPosition.y, -7.0f);
