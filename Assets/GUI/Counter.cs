  ...
  //  UILabel label;
	...
	void Update () {
        if (name == "Moves")
        {
            ...
            if( LevelData.LimitAmount <= 5 && BubbleGamePlay.Instance.GameStatus == BubbleGameState.Playing )
            {
                label.color = Color.red;
                if( !GetComponent<Animation>().isPlaying )
                {
                    GetComponent<Animation>().Play();
                    AudioManager.Instance.playerEffect1(SoundBase.Instance.alert);
                }
            }
        }
